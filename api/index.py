...
# ----------------------------------------------------
# 1. უსაფრთხოება და Rate Limiting კონფიგურაცია
# ----------------------------------------------------
API_AUTH_TOKEN = os.getenv("API_AUTH_TOKEN", "Bearer LINGOLENS_SECURE_TOKEN_2026")
REQUEST_LIMIT = 40  # მაქსიმუმ 40 მოთხოვნა წუთში თითო IP-ზე
WINDOW_TIME = 60
ip_requests = {}
...
def is_rate_limited(client_ip: str) -> bool:
    ...
    # 60 წამზე ძველი მოთხოვნების გასუფთავება
    ip_requests[client_ip] = [t for t in ip_requests[client_ip] if current_time - t < WINDOW_TIME]
    
    if len(ip_requests[client_ip]) >= REQUEST_LIMIT:
        return True
...
async def security_and_rate_limit_middleware(request: Request, call_next):
    # Root და Health-check ენდპოინტებზე ავტორიზაციის გამოტოვება
    if request.url.path in ["/", "/api/index"] and request.method == "GET":
    ...
    # 2. Bearer Token Auth შემოწმება
    auth_header = request.headers.get("Authorization")
    if not auth_header or auth_header != API_AUTH_TOKEN:
...
# ----------------------------------------------------
# 2. მონაცემთა მოდელები და რუკები
# ----------------------------------------------------
...
def analyze_grammar(text: str, src_lang: str, tgt_lang: str) -> str:
    ...
    analysis_lines = [
        "📊 [გრამატიკული და სტრუქტურული ანალიზი]",
