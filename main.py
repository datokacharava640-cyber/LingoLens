# ==============================================================================
# LingoLens AI - Main App (Fixed Vercel Backend URL & World Languages)
# ==============================================================================
...
class LingoLensApp(App):
    ...
    def build(self):
        ...
        self.ensure_backend_url()
        self.show_main_menu()
        return self.root_layout

    def ensure_backend_url(self):
        """Yemeza neza ko Vercel URL buri gihe iba yanditse muri settings na config"""
        url_to_set = DEFAULT_BACKEND_URL
        if os.path.exists(SETTINGS_FILE):
            try:
                with open(SETTINGS_FILE, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    if not data.get("api_key"):
                        data["api_key"] = url_to_set
            ...
            try:
                with open(SETTINGS_FILE, "w", encoding="utf-8") as f:
                    json.dump({"api_key": url_to_set}, f)
            except:
                pass

        if config:
            if not getattr(config, 'GEMINI_API_KEY', None):
                config.GEMINI_API_KEY = url_to_set
    ...
    def switch_to_view(self, widget):
        ...
        btn_back = Button(text=self.get_text('back_btn') if self.get_text('back_btn') != 'back_btn' else "უკან", font_name=font, size_hint_x=0.3, on_press=self.show_main_menu)
    ...
    def open_translator(self, instance=None, initial_text="", initial_result="", status_text=""):
        ...
        # Indimi z'isi zose zifatika (World Languages)
        world_languages = {
            "ქართული": "ka", 
            "English": "en", 
            "Русский": "ru", 
        ...
        lang_inv_names = {v: k for k, v in world_languages.items()}
        
        sp_src = Spinner(text=lang_inv_names.get(self.current_lang, "ქართული"), values=list(world_languages.keys()), font_name=font)
        sp_target = Spinner(text="English", values=list(world_languages.keys()), font_name=font)
        ...
        btn_listen = Button(text="მოსმენა", font_name=font, on_press=lambda x: self.safe_speak(out.text, world_languages.get(sp_target.text, "en")))
        ...
        def handle_translation(btn_inst):
            ...
            src_code = world_languages.get(sp_src.text, "ka")
            target_code = world_languages.get(sp_target.text, "en")
    ...
    def open_settings(self, instance):
        font = FONT_NAME
        box = BoxLayout(orientation='vertical', padding=10, spacing=10, size_hint=(1, 0.9))
        lbl = Label(text="Vercel Backend URL:", font_name=font, size_hint_y=0.15)
        
        current_key = DEFAULT_BACKEND_URL
        if os.path.exists(SETTINGS_FILE):
            try:
                with open(SETTINGS_FILE, "r", encoding="utf-8") as f:
                    current_key = json.load(f).get("api_key", DEFAULT_BACKEND_URL)
            except:
                pass

        inp_key = TextInput(text=current_key, hint_text="URL...", font_name=font, multiline=False)
