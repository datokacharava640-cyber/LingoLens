...
AUTH_TOKEN = "lingolens_secure_token_2026"
...
class NetworkIndicator(Widget):
    def set_status(self, status):
        ...
        with self.canvas.before:
            if status == "green":
                Color(0.1, 0.8, 0.2, 1)  # მწვანე
            elif status == "yellow":
                Color(0.9, 0.7, 0.1, 1)  # ყვითელი
            else:
                Color(0.9, 0.2, 0.2, 1)  # წითელი
...
class AsyncTranslateEngine:
    @staticmethod
    def async_post_request(url, payload, callback, headers=None):
        def _worker():
            req_headers = {
...
class MainScreen(Screen):
    ...
    def check_network_status(self, dt):
        def _check():
            try:
                res = requests.get(f"{VERCEL_BASE_URL}/", timeout=3)
    ...
    def _delayed_translate(self, text, callback_after=None):
        url = f"{VERCEL_BASE_URL}/api/v1/translate"
        payload = {
            "text": text,
