...
# =====================================================================
# 1. UI დიზაინის შაბლონები (Modern Glass Dark Theme)
# =====================================================================
KV_DESIGN = """
<RoundedButton>:
...
class MainLayout(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Status Bar
        status_txt = "Gemini AI აქტიურია | მზადაა" if self.api_key else "მზადაა | 18 AI მოდული ჩატვირთულია"
        ...
        btn_trans = RoundedButton(text="AI თარგმნა", bg_color=(0.12, 0.55, 0.38, 1), font_name=GEORGIAN_FONT if GEORGIAN_FONT else None)
        btn_trans.bind(on_press=self.translate_text)

        btn_tts = RoundedButton(text="წაკითხვა", bg_color=(0.2, 0.45, 0.75, 1), font_name=GEORGIAN_FONT if GEORGIAN_FONT else None)
        btn_tts.bind(on_press=self.speak_georgian)

        btn_copy = RoundedButton(text="დაკოპირება", bg_color=(0.28, 0.35, 0.45, 1), font_name=GEORGIAN_FONT if GEORGIAN_FONT else None)
        ...
        self.menu_btn = RoundedButton(
            ...
            bg_color=(0.16, 0.22, 0.32, 1),
            size_hint_y=0.08,
            font_name=GEORGIAN_FONT if GEORGIAN_FONT else None
        )
        self.dropdown = DropDown(max_height=260)  # ლიმიტი სიმაღლეზე, რომ ეკრანს არ გასცდეს

        modules_list = [
            ("Next-Gen Features (5 ინოვაცია)", "next_gen_features"),
            ("Super Features (5 ფუნქცია)", "super_features"),
    ...
    def run_module(self, module_name):
        try:
            ...
            if module_name == "next_gen_features" and hasattr(mod, "open_next_gen_features"):
                ...
                self.status_label.text = "Next-Gen Features გახსნილია!"
                return

            if module_name == "super_features" and hasattr(mod, "open_super_features"):
                mod.open_super_features(self)
                self.status_label.text = "Super Features გახსნილია!"
                return
