...
# მკაფიო და უსაფრთხო სია (პირველი ელემენტია ინგლისური, რომ კვადრატები არ აიჭრას)
SUPPORTED_LANGUAGES = {
    "English": "en",
    "ქართული": "ka", 
    "Русский": "ru", 
...
class LingoLensApp(App):
    ...
    def build(self):
        ...
        # ასინქრონულად ვწერთ კონფიგურაციას ფონურ რეჟიმში, რომ არ გაიჭედოს
        Clock.schedule_once(lambda dt: self.force_backend_url(), 0.1)
        
        # მენიუ იხსნება მყისიერად
        self.show_main_menu()
        return self.root_layout

    def force_backend_url(self):
        """ძალისმიერად უწერს Vercel URL-ს როგორც config-ს, ისე settings ფაილს უსაფრთხოდ"""
        try:
            if config:
    ...
    def open_settings(self, instance):
        ...
        box.add_widget(Label(text=f"Backend URL:\n{DEFAULT_BACKEND_URL}", font_name=font, size_hint_y=0.3))
        self.switch_to_view(box)
