...
class LingoLensRealTimeUI(BoxLayout):
    ...
    def open_camera_ocr(self, instance):
        content = BoxLayout(orientation='vertical', padding=10, spacing=10)
        content.add_widget(Label(text="OCR Camera ready for scan"))
        ...
        self.cam_popup = Popup(title="AI OCR Scanner", content=content, size_hint=(0.85, 0.4))
        self.cam_popup.open()

    def _process_ocr_scan(self, instance):
        if hasattr(self, 'cam_popup'):
            self.cam_popup.dismiss()
        self.result_label.text = "Set API Key in Settings for OCR."
