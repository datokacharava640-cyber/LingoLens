...
# =====================================================================
# 1. ქართული შრიფტი (NotoSansGeorgian.ttf)
# =====================================================================
GEORGIAN_FONT = None
...
# =====================================================================
# 2. მთავარი აპლიკაციის ლოგიკა და ცოცხალი Ecosystem
# =====================================================================
class MainLayout(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Android-ის ნატიური TTS ინიციალიზაცია
        if platform == 'android':
        ...
        # Status Bar (ცოცხალი სტატუსი)
        self.status_label = Label(
            text="🔴 იღვიძებს... ველოდები ნებართვებს / დაძახებას",
            color=(0.9, 0.6, 0.2, 1),
            font_size='13sp',
            size_hint_y=0.05
        )
        ...
        self.output_label = Label(
            text="[AI პასუხი / თარგმანი გამოჩნდება აქ]",
            markup=True,
            font_size='15sp',
            size_hint_y=0.2,
            color=(0.9, 0.9, 0.9, 1)
        ...
        # -------------------------------------------------------------
        # Action Buttons Grid (ყველა მოდული)
        # -------------------------------------------------------------
        ...
        self.btn_live.bind(on_press=self.toggle_live_interpreter)
    ...
    # =================================================================
    # background Wake-Word & Live Listening Thread
    # =================================================================
    def start_background_wake_word(self):
        """აპლიკაციის ჩართვისთანავე უსაფრთხოდ ააქტიურებს wake_word.py-ს"""
        def listen_loop():
            try:
                sys.path.append(os.path.join(os.path.dirname(__file__), 'modules'))
                import wake_word
                import live_interpreter
                Clock.schedule_once(lambda dt: self.update_status("🟢 უსმენს დაძახებას (Wake Word აქტიურია)", (0.2, 0.8, 0.2, 1)))
            except Exception as e:
                print(f"Wake Word Thread Warning: {e}")
                Clock.schedule_once(lambda dt: self.update_status("🟢 მზადაა | Wake Word მოდული ჩატვირთულია", (0.2, 0.8, 0.2, 1)))
    ...
    def translate_text(self, instance):
        ...
        self.update_status("AI თარგმნის...", (0.2, 0.6, 0.9, 1))
        ...
        if self.api_key:
            try:
                ...
                if res.status_code == 200:
                    ...
                    self.update_status("Gemini AI თარგმანი მზადაა!", (0.2, 0.8, 0.2, 1))
                    self.speak_georgian(None)  # ავტომატური ხმოვანი გაჟღერება
                    return
            except Exception as e:
        ...
        try:
            ...
            self.update_status("თარგმანი მზადაა (Free Mode)", (0.2, 0.8, 0.2, 1))
            self.speak_georgian(None)  # ავტომატური ხმოვანი გაჟღერება
        except Exception as e:
    ...
    def speak_georgian(self, instance):
        ...
        if platform == 'android' and self.tts:
            try:
                ...
                self.update_status("🔊 რობოტი გპასუხობს...", (0.2, 0.8, 0.8, 1))
            except Exception as e:
    ...
    def toggle_live_interpreter(self, instance):
        try:
            sys.path.append(os.path.join(os.path.dirname(__file__), 'modules'))
            import live_interpreter
            self.update_status("🎙️ Live Interpreter აქტიურია!", (0.2, 0.8, 0.2, 1))
        except Exception as e:
            self.update_status(f"Live Error: {e}")

    def run_ar_camera(self, instance):
        try:
            sys.path.append(os.path.join(os.path.dirname(__file__), 'modules'))
            import ar_camera
            self.update_status("📷 AR Camera ჩაირთო!", (0.2, 0.8, 0.2, 1))
        except Exception as e:
        ...
        if platform == 'android':
            try:
                ...
                if not Settings.canDrawOverlays(activity):
                    ...
                    self.update_status("მიანიჭეთ Floating Permission...", (0.9, 0.6, 0.2, 1))
                    return

                sys.path.append(os.path.join(os.path.dirname(__file__), 'modules'))
                import floating_bubble
                self.update_status("💬 Floating Bubble გააქტიურებულია!", (0.2, 0.8, 0.2, 1))
            except Exception as e:
                self.update_status(f"Bubble Error: {e}")
        else:
            self.update_status("Requires Android Overlay Environment")

    def run_voice_module(self, instance):
        try:
            sys.path.append(os.path.join(os.path.dirname(__file__), 'modules'))
            import voice_clone
            import walkie_talkie
            self.update_status("🗣️ Voice Clone & Walkie ჩაირთო!", (0.2, 0.8, 0.2, 1))
        except Exception as e:
    ...
    def run_doc_summarizer(self, instance):
        try:
            ...
            self.update_status("📄 Doc Summarizer ჩაირთო!", (0.2, 0.8, 0.2, 1))
        except Exception as e:
            self.update_status(f"Doc Error: {e}")
    ...
    def open_api_popup(self, instance):
        ...
        def save_key(btn_instance):
            ...
            self.update_status("Gemini AI რობოტი გააქტიურდა!", (0.2, 0.8, 0.2, 1))
            popup.dismiss()
        ...
        try:
            ...
            def permissions_callback(permissions, results):
                if all(results):
                    ...
                    self.main_layout.update_status("⚠️ ნებართვები საჭიროა სრული ფუნქციონალისთვის")
