...
# შრიფტის უსაფრთხო რეგისტრაცია (არ იწვევს Crash-ს)
CUSTOM_FONT = None
if os.path.exists('font.ttf'):
    try:
        if os.path.getsize('font.ttf') > 10000:
            LabelBase.register(name='GeoFont', fn_regular='font.ttf')
            CUSTOM_FONT = 'GeoFont'
...
class LingoLensUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        font_kwargs = {'font_name': CUSTOM_FONT} if CUSTOM_FONT else {}
        ...
        btn_ai = Button(text='🧠 AI თარგმნა', background_color=(0.1, 0.5, 0.9, 1), **font_kwargs)
        btn_ai.bind(on_press=self.start_translation)
        grid.add_widget(btn_ai)

        btn_voice = Button(text='🎙️ ხმოვანი რეჟიმი', background_color=(0.1, 0.7, 0.3, 1), **font_kwargs)
        btn_voice.bind(on_press=self.voice_action)
        grid.add_widget(btn_voice)

        btn_cam = Button(text='📷 კამერა / OCR', background_color=(0.9, 0.4, 0.1, 1), **font_kwargs)
        btn_cam.bind(on_press=self.camera_action)
        grid.add_widget(btn_cam)

        btn_mode = Button(text='🪟 Overlay რეჟიმი', background_color=(0.6, 0.2, 0.8, 1), **font_kwargs)
        ...
        self.result_label = Label(
            text='LingoLens მზადაა სამუშაოდ.',
            font_size='16sp',
            size_hint_y=0.33,
            **font_kwargs
        )
