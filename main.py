...
<MainScreen>:
    ...
    BoxLayout:
        ...
        BoxLayout:
            ...
            TextInput:
                id: input_text
                hint_text: "✏️ ჩაწერეთ ტექსტი ან გამოიყენეთ ხმოვანი ღილაკი..."
                font_name: 'font.ttf'
    ...
    def _send_frame_to_cloud(self, pixels, w, h):
        try:
            ...
            def _on_success(req, res):
                ...
                Clock.schedule_once(lambda dt: setattr(self.ar_label, 'text', text if text else "[⚠️ ტექსტი ვერ მოიძებნა]"), 0)
                self.is_processing = False

            def _on_error(req, err):
                Clock.schedule_once(lambda dt: setattr(self.ar_label, 'text', "[⚠️ სერვერის შეცდომა / Offline]"), 0)
                self.is_processing = False
...
class MainScreen(Screen):
    ...
    def copy_output_text(self):
        ...
        if text:
            Clipboard.copy(text)
            self.show_toast("📋 ტექსტი დაკოპირდა!")
    ...
    def open_language_menu(self, mode='source'):
        ...
        popup = Popup(title=f"🌐 ენის არჩევა ({'საწყისი' if mode == 'source' else 'სამიზნე'})", content=box, size_hint=(0.85, 0.8))

        for name, code in LANGUAGES.items():
            btn_flag = self._get_flag(code)
            btn = Button(text=f"{btn_flag}{name}", font_name=FONT_PATH, size_hint_y=None, height='40dp', background_color=(0.15, 0.2, 0.3, 1))
            def _select(instance, c=code, n=name, f=btn_flag):
                if mode == 'source':
                    self.source_lang = c
                    self.ids.btn_source_lang.text = f"🌐 {f}{n}"
                else:
                    self.target_lang = c
                    self.ids.btn_target_lang.text = f"🌐 {f}{n}"
                popup.dismiss()
    ...
    def swap_languages(self):
        ...
        src_flag = self._get_flag(self.source_lang)
        tgt_flag = self._get_flag(self.target_lang)

        self.ids.btn_source_lang.text = f"🌐 {src_flag}{src_name}"
        self.ids.btn_target_lang.text = f"🌐 {tgt_flag}{tgt_name}"
    ...
    def open_main_menu(self):
        ...
        popup = Popup(title="⚙️ მენიუ", content=box, size_hint=(0.8, 0.5))
        
        btn_history.bind(on_release=lambda x: (popup.dismiss(), self.open_history_view()))
        btn_clear.bind(on_release=lambda x: (self.db.clear_history(), popup.dismiss(), self.show_toast("🗑️ ისტორია გასუფთავდა")))
    ...
    def open_conversation_mode(self):
        ...
        self.conv_label = Label(
            text="[🎙️ ორმხრივი დიალოგი მზადაა]\nდააჭირეთ '▶️ Start Live Loop'-ს საუბრისთვის.",
        ...
        close_btn = Button(text="⏹️ დასრულება", font_name=FONT_PATH, size_hint_y=None, height='40dp', background_color=(0.3, 0.3, 0.3, 1))
        box.add_widget(close_btn)

        self.conv_popup = Popup(title="🎙️ Real-Time Conversation", content=box, size_hint=(0.95, 0.85))
    ...
    def start_speech_to_text(self, lang_code, target_field='input'):
        ...
        if platform == 'android':
            try:
                ...
                if target_field == 'input': self.ids.input_text.text = f"[⚠️ STT Error: {e}]"
        else:
