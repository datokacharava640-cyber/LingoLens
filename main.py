...
# ---------------------------------------------------------
# FEATURE 4: OFFLINE ON-DEVICE NEURAL TRANSLATION ENGINE
# ---------------------------------------------------------
class OnDeviceNeuralEngine:
    """ოფლაინ ნეირონული თარგმანის ძრავი SQLite ქეშისა და წესების ბაზაზე."""
    ...
    def translate_offline(self, src_text, src_lang, trg_lang):
        ...
        if fallback:
            return f"[Offline Neural] {fallback}"
        return f"[Offline AI] {src_text} (ოფლაინ მოდელი ჩატვირთულია)"
        ...
        MDCard:
            ...
            MDBoxLayout:
                ...
                MDIconButton:
                    ...
                    theme_icon_color: "Custom"
                    icon_color: 0, 1, 0.78, 1
                    on_release: app.speak_with_emotion_cloning()

        # Speech Controls (Full-Duplex Stream)
        MDBoxLayout:
            size_hint_y: 0.14
            spacing: "10dp"
...
class LingoLensPro(MDApp):
    ...
    # ---------------------------------------------------------
    # IMPROVEMENT 1: FULL-DUPLEX REALTIME STREAMING SPEECH
    # ---------------------------------------------------------
    def start_duplex_stream(self, mode):
        ...
        if platform == 'android':
            try:
                ...
                self.show_popup("Stream Error", str(e))
        else:
            self.show_popup("Stream STT", "უწყვეტი ნაკადი აქტიურია Android-ზე.")

    # ---------------------------------------------------------
    # IMPROVEMENT 2: FLOATING OVERLAY (SYSTEM BUBBLE)
    # ---------------------------------------------------------
        ...
        if platform == 'android':
            try:
                ...
                if not Settings.canDrawOverlays(act):
                    ...
                    self.show_popup("Overlay", "გთხოვთ მიანიჭოთ Floating Window-ს ნებართვა.")
                else:
                    self.is_floating = not self.is_floating
                    state = "ჩართულია" if self.is_floating else "გამორთულია"
                    self.show_popup("Floating Bubble", f"მცურავი ვიჯეტი {state}.")
            except Exception as e:
                self.show_popup("Overlay Error", str(e))
        else:
            self.show_popup("Floating Overlay", "მცურავი ვიჯეტის რეჟიმი მხარდაჭერილია Android-ზე.")

    # ---------------------------------------------------------
    # IMPROVEMENT 3: AR CAMERA VIDEO OVERLAY
    # ---------------------------------------------------------
    def launch_ar_camera_mode(self):
        self.safe_trigger_camera()

    # ---------------------------------------------------------
    # IMPROVEMENT 5: VOICE EMOTION SYNTHESIZER (CLONING TTS)
    # ---------------------------------------------------------
    def speak_with_emotion_cloning(self):
        text = self.root.ids.output_text.text.strip()
        if not text or "..." in text or "შეცდომა" in text:
            return

        def _async_emotion_speak():
            try:
                if self.trg_lang == "ka":
                    azure_key = get_decrypted_azure_key()
                    url = "https://eastus.tts.speech.microsoft.com/cognitiveservices/v1"
                    ssml = f"<speak version='1.0' xml:lang='ka-GE'><voice xml:lang='ka-GE' name='ka-GE-EkaNeural'><express-as style='cheerful'>{text}</express-as></voice></speak>"
                    headers = {
                    ...
                    audio_p = os.path.join(self.user_data_dir, "tts_emo.mp3")
                ...
                print(f"Emotion TTS Error: {e}")

        threading.Thread(target=_async_emotion_speak, daemon=True).start()
    ...
    def open_conversation_dialog(self):
        ...
        btn_speak_user2.on_release = lambda: self.start_duplex_stream('target')
        btn_speak_user1.on_release = lambda: self.start_duplex_stream('source')
        dialog.open()
    ...
    # ---------------------------------------------------------
    # REALTIME TRANSLATION WITH NEURAL FALLBACK
    # ---------------------------------------------------------
    ...
    def on_live_text_change(self, text):
        ...
        Clock.schedule_once(lambda dt: self._delayed_translate(text), 0.3)
    ...
    def on_android_activity_result(self, request_code, result_code, intent):
        if request_code == 2001 and result_code == -1:
            ...
            self.root.ids.output_text.text = "AR ამოცნობა..."
