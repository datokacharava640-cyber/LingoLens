...
class LingoLensUI(BoxLayout):
    def __init__(self, db, **kwargs):
        ...
        btn_history = Button(text="History", size_hint_x=0.3)
        ...
        self.btn_swap = Button(text="⇄", size_hint_x=0.2, font_size='20sp', bold=True)
    ...
    def on_translation_success(self, req, result):
        try:
            ...
            self.output_label.text = "[Error parsing response]"

    def on_translation_error(self, req, error):
        self.output_label.text = "[Network error]"

    def copy_to_clipboard(self, instance):
        if self.output_label.text and "[Error]" not in self.output_label.text:
    ...
    def open_history(self, instance):
        ...
        popup = Popup(title="History", content=content, size_hint=(0.9, 0.75))
        popup.open()
