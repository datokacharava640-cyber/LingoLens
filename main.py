...
# Android-ის მიკროფონისთვის სრული ლოკალები (STT)
STT_LOCALES = {
    "auto": "en-US",
    "en": "en-US",
    "ka": "ka-GE",
...
class LingoLensUI(BoxLayout):
    ...
    def listen_foreigner(self, instance):
        # მარცხენა სპინერში არჩეული უცხოური ენის მიხედვით მოსმენა
    ...
    def listen_georgian(self, instance):
        # თუ ქართულად ვსაუბრობთ, სპინერებში ენებს ვცვლით (ქართული -> უცხოური)
        if self.src_spinner.text != "ქართული":
            current_src = self.src_spinner.text
            current_target = self.target_spinner.text
