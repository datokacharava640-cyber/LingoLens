...
class TranslatorScreen(Screen):
  ...
  def speak_result(self, instance):
    ...
    # 1. Android Native TTS (Pyjnius-ის მეშვეობით)
    try:
      ...
      def on_init(status):
        if status == TextToSpeech.SUCCESS:
          loc = Locale(self.target_lang)
          self.tts_engine.setLanguage(loc)
          self.tts_engine.speak(
              text_to_speak, TextToSpeech.QUEUE_FLUSH, None, None
          )

      self.tts_engine = TextToSpeech(activity, on_init)
      return
    except Exception:
      pass

    # 2. Plyer რეზერვი (თუ Pyjnius მიუწვდომელია)
    try:
      if tts:
