    ...
    try:
        ...
        class ContinuousSpeechListener(PythonJavaClass):
            ...
            def onEndOfSpeech(self):
                # პაუზის შემდეგ ავტომატური რესტარტი უწყვეტი მოსმენისთვის
                if self.restart_callback:
                    self.restart_callback()

            @java_method('(I)V')
            def onError(self, error):
                # შეცდომის/Timeout-ის დროს ავტომატური რესტარტი
                if self.restart_callback:
                    self.restart_callback()
...
class LingoLensRealTimeUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # 1. Header & API Settings
        header_box = BoxLayout(orientation='horizontal', size_hint_y=0.08)
        header_box.add_widget(Label(text='LingoLens Real-Time AI', font_size='18sp', bold=True))
        
        btn_key = Button(text='⚙️ API Key', size_hint_x=0.3, background_color=(0.4, 0.4, 0.4, 1))
        ...
        self.input_text = TextInput(
            hint_text='Type text or talk live below...',
            font_size='15sp',
            multiline=True,
            size_hint_y=0.25
        )
        ...
        # 4. Live 2-Way Speech Buttons
        talk_box = BoxLayout(orientation='horizontal', spacing=8, size_hint_y=0.12)
        self.btn_p1 = Button(text='🎙️ Person 1 Live', background_color=(0.1, 0.6, 0.3, 1))
        self.btn_p1.bind(on_press=lambda inst: self.toggle_voice_mode('P1'))
        self.btn_p2 = Button(text='🎙️ Person 2 Live', background_color=(0.8, 0.4, 0.1, 1))
        ...
        # 6. Result Output
        label_kwargs = {
            'text': 'Ready for Continuous Real-Time Live Speech.',
            'font_size': '15sp',
            'size_hint_y': 0.35
        }
        if GEORGIAN_FONT:
    ...
    # Continuous Auto-Restart Voice Loop
    def toggle_voice_mode(self, person):
        ...
        if self.is_listening and self.current_person == person:
            ...
            if person == 'P1':
                self.btn_p1.text = "Listening P1... ⏹️"
                lang_code = LANGUAGES.get(self.src_spinner.text, 'en')
            else:
                self.btn_p2.text = "Listening P2... ⏹️"
        ...
        if is_internet_available() and os.path.exists("ocr_frame.png") and GEMINI_API_KEY != "YOUR_GEMINI_API_KEY":
            try:
                ...
                data = json.dumps({
                    "contents": [{
                        "parts": [
                            {"text": "Extract all text directly from this image."},
            ...
            Clock.schedule_once(lambda dt: self._set_ocr_text("Frame Captured."))
