# ==============================================================================
# LingoLens Ultra Pro v3.2.1 🇬🇪 - Truly Real-Time Autonomous Engine
# ==============================================================================
...
ADVANCED_OFFLINE_DB = {
    ("ka", "en_US"): {
        ...
        "რა ღირს": "How much does it cost", "ნახვამდის": "Goodbye", "დიახ": "Yes", "არა": "No"
    },
    ("en_US", "ka"): {
        "hello": "გამარჯობა", "thank you": "მადლობა", "how are you": "როგორ ხარ",
        "i love programming": "მე მიყვარს პროგრამირება", "where is the hotel": "სად არის სასტუმრო",
        "how much is it": "რა ღირს ეს", "goodbye": "ნახვამდის", "yes": "დიახ", "no": "არა"
    }
}
...
class CameraARWidget(BoxLayout):
    ...
    def update_frame(self, dt):
        ...
        if not self.is_processing:
            ...
            threading.Thread(target=self._process_ar_ocr, args=(frame.copy(),), daemon=True).start()

        for (x, y, w, h, text) in self.processed_boxes:
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(frame, text, (x, max(y - 8, 15)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    def _process_ar_ocr(self, frame):
        try:
            ...
            for i in range(len(data['text'])):
                ...
                if conf > 60 and len(text) > 1:
                    ...
                    translated = self.main_screen.smart_offline_translate(text)
                    new_boxes.append((x, y, w, h, translated))
            self.processed_boxes = new_boxes
        except Exception:
            pass
        finally:
            self.is_processing = False
...
class MainScreen(Screen):
    ...
    # ==========================================================================
    # Autonomous Real-Time Live Conversation (Auto Turn-Taking Loop)
    # ==========================================================================
    def open_conversation_mode(self):
        ...
        self.conv_label = Label(
            text="[ავტონომიური Real-Time დიალოგი მზადაა]\nდააჭირეთ 'Live Auto Loop' უწყვეტი რეჟიმისთვის.",
            font_name=FONT_PATH,
            size_hint_y=None,
            markup=True,
            color=(1, 1, 1, 1),
        ...
        self.conv_popup = Popup(title="Real-Time Autonomous Conversation", content=box, size_hint=(0.95, 0.85))
    ...
    def toggle_auto_dialog_loop(self, instance):
        ...
        if self.is_auto_loop_active:
            ...
            self.btn_auto.text = "▶️ Auto Loop (უწყვეტი)"
    ...
    def _add_to_dialog_ui(self, original, translated, speaker, speak_lang):
        speaker_name = "მოსაუბრე 1 (ქართული)" if speaker == 'dialog_a' else "მოსაუბრე 2 (უცხო ენა)"
    ...
    # ==========================================================================
    # Native Android Speech Recognizer integration
    # ==========================================================================
    def start_speech_to_text(self, lang_code, target_field='input'):
        if platform == 'android':
            ...
            dummy = "გამარჯობა, სად არის უახლოესი სასტუმრო?" if "ka" in lang_code else "Hello, where is the hotel?"
            if target_field == 'input':
                self.ids.input_text.text = dummy
            elif 'dialog' in target_field:
                self.handle_conversation_speech(dummy, target_field)

    # ==========================================================================
    # Real-Time UI & Gemini Integration
    # ==========================================================================
    def translate_with_grammar(self):
        ...
        UrlRequest(
            ...
            on_error=lambda req, err: setattr(self.ids.output_text, 'text', f"[Offline Engine]\n{self.smart_offline_translate(text)}"),
            timeout=7
        )
    ...
    def open_ar_camera_mode(self):
        ...
        box.add_widget(close_btn)
        popup = Popup(title="Live AR Scanner", content=box, size_hint=(0.95, 0.9))
