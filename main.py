    ...
    def _init_android_tts(self):
        if platform == 'android':
            try:
                ...
                class TTSOnInitListener(autoclass('android.speech.tts.TextToSpeech$OnInitListener')):
                ...
                listener = TTSOnInitListener(self)
                self.tts = TextToSpeech(PythonActivity.mActivity, listener)
            except Exception as e:
    ...
    def speak_output_text(self, custom_text=None):
        ...
        if platform == 'android' and self.tts:
            try:
                ...
                self.tts.setLanguage(locale_obj)
                self.tts.speak(text, TextToSpeech.QUEUE_FLUSH, None, None)
                return
            except Exception as e:
                print(f"Android Native TTS Error: {e}")

        threading.Thread(target=self._play_fallback_audio, args=(text, lang_code), daemon=True).start()

    def _play_fallback_audio(self, text, lang_code):
        try:
            base_dir = "."
