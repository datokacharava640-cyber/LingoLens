...
# Safe Plyer Import
try:
    from plyer import filechooser, clipboard, tts
except Exception:
    filechooser = None
    clipboard = None
    tts = None
...
class SafeCacheManager:
    ...
    def get_history(self):
        try:
            with sqlite3.connect(self.db_path, check_same_thread=False) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT src, trg FROM history ORDER BY id DESC LIMIT 25")
                return cursor.fetchall()
        except Exception:
            return []

# მსოფლიოს გაფართოებული ენების სია
LANGUAGES = {
    ...
    "Norsk": {"code": "no", "stt": "no-NO"},
    "Dansk": {"code": "da", "stt": "da-DK"},
    "Suomi": {"code": "fi", "stt": "fi-FI"},
    "Čeština": {"code": "cs", "stt": "cs-CZ"},
    "Magyar": {"code": "hu", "stt": "hu-HU"},
    "Română": {"code": "ro", "stt": "ro-RO"},
    "Български": {"code": "bg", "stt": "bg-BG"},
    "Azərbaycan": {"code": "az", "stt": "az-AZ"},
    "Հայերեն (სომხური)": {"code": "hy", "stt": "hy-AM"},
    "Қазақ (ყაზახური)": {"code": "kk", "stt": "kk-KZ"},
    "Uzbek": {"code": "uz", "stt": "uz-UZ"},
    "فارسی (სპარსული)": {"code": "fa", "stt": "fa-IR"},
    "Tiếng Việt (ვიეტნამური)": {"code": "vi", "stt": "vi-VN"},
    "ไทย (ტაილანდური)": {"code": "th", "stt": "th-TH"},
    "Bahasa Indonesia": {"code": "id", "stt": "id-ID"}
}
...
class LingoLensPro(MDApp):
    ...
    def _delayed_translate(self, text):
        current_hash = hashlib.md5(f"{text}_{self.src_lang}_{self.trg_lang}".encode('utf-8')).hexdigest()
    ...
    # ტექსტის გაჟღერება (TTS / SMS გახმოვანება)
    def speak_translated_text(self):
        text = self.root.ids.output_text.text
        if not text or text == "ნათარგმნი ტექსტი...":
            return
        ...
        if platform == 'android':
            try:
                ...
                loc = Locale(self.trg_lang)
                tts_engine.setLanguage(loc)
    ...
    def start_duplex_stream(self, mode):
        if platform == 'android':
            try:
                ...
                stt_code = LANGUAGES.get(self.root.ids.btn_src.text, {}).get("stt", "ka-GE")
