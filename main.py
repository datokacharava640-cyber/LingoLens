...
Version             : 4.0.0 (Dual-Voice Dialogue & Live OCR with Gemini AI)
...
class LingoLensApp(App):
    ...
    def build(self):
        ...
        # Status Bar
        self.status = Label(
            text="მზადაა დიალოგისთვის (Gemini AI Powered)", size_hint_y=0.04, font_size='12sp', font_name=font_to_use
        )
    ...
    def toggle_recording(self, speaker):
        ...
        if not self.is_recording:
            ...
            try:
                ...
                self.is_recording = False
                self.set_mic_buttons_state(disabled=True)
                self.status.text = "⏳ მუშავდება და ითარგმნება..."
                threading.Thread(target=self.send_audio_task, args=(src, tgt), daemon=True).start()
            except Exception as e:
                self.set_mic_buttons_state(disabled=False)
                self.reset_mic_buttons()
                self.status.text = f"ჩაწერის გაჩერების შეცდომა: {e}"

    def set_mic_buttons_state(self, disabled=False):
        self.btn_mic_a.disabled = disabled
        self.btn_mic_b.disabled = disabled
    ...
    def send_audio_task(self, src, tgt):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                self.update_status(f"სერვერის შეცდომა: HTTP {res.status_code}")
        except requests.exceptions.Timeout:
            self.update_status("❌ მოთხოვნის დრო ამოიწურა (Timeout)")
        except requests.exceptions.ConnectionError:
            self.update_status("❌ ინტერნეტის კავშირი შეწყვეტილია")
        except Exception as e:
    ...
    def translate_task(self, text):
        ...
        def async_run():
            ...
            # 1. პრიორიტეტი: პირდაპირ Gemini AI-ს გამოყენება (თუ KEY გაწერილია)
            if HAS_GEMINI:
                try:
                    prompt = f"Translate the following text from {src_lang} to {tgt_lang}. Output ONLY the raw translated text without explanation:\n\n{text}"
                    response = gemini_model.generate_content(prompt)
                    if response and response.text:
                        self.update_out(response.text.strip())
                        self.update_status("✅ თარგმანი Gemini AI-ს მიერ")
                        return
                except Exception as e:
                    print(f"[Gemini Translate Fallback]: {e}")

            # 2. სათადარიგო: API სერვერთან დაკავშირება
            try:
                res = requests.post(
                ...
                if res.status_code == 200:
                    ...
                    self.update_status("✅ თარგმანი API-ს მიერ")
    ...
    def capture_frame(self, dt):
        ...
        try:
            ...
            def async_ocr(data_bytes):
                ...
                # 1. პრიორიტეტი: Gemini Vision (Direct Image OCR & Translation)
                if HAS_GEMINI:
                    try:
                        pil_img = PILImage.open(io.BytesIO(data_bytes))
                        prompt = (
                            f"Detect any text in this image. "
                            f"Translate it to {tgt_lang}. "
                            f"Format output as JSON with two keys: 'extracted_text' and 'translated_text'."
                        )
                        response = gemini_model.generate_content([prompt, pil_img])
                        if response and response.text:
                            # მარტივი პარსინგი Gemini-ს პასუხისთვის
                            resp_text = response.text.strip()
                            self.apply_ocr("Gemini OCR Image Detected", resp_text)
                            return
                    except Exception as e:
                        print(f"[Gemini Vision Fallback]: {e}")

                # 2. სათადარიგო: API სერვერზე გაგზავნა
                try:
                    res = requests.post(
                        f"{API_URL}/ocr-translate",
                        files={'file': ('frame.jpg', io.BytesIO(data_bytes), 'image/jpeg')},
                        data={'target_lang': tgt_lang},
                        timeout=10
                    )
                    if res.status_code == 200:
                        data = res.json()
                    ...
                    print("[OCR Error]: No internet connection")
                except Exception as e:
                    print(f"[OCR Error]: {e}")
                finally:
                    self.ocr_in_progress = False
    ...
    def speak_text(self, instance):
        if HAS_PLYER and self.output_text.text:
            try:
                ...
                self.status.text = f"TTS შეცდომა: {e}"
