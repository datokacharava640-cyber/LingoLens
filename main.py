...
class LingoLensUI(BoxLayout):
    ...
    def speak_georgian_azure(self, text):
        def download_and_play():
            try:
                ...
                with urllib.request.urlopen(req) as response, open(audio_path, 'wb') as f:
    ...
    def speak_text(self, instance):
        ...
        lang_code = getattr(self, 'last_target_lang', 'en')

        # თუ ენა ქართულია, ვიყენებთ Azure-ს
        if lang_code == 'ka':
        ...
        if platform == 'android':
            try:
                ...
                self.media_player = MediaPlayer()
                tts_url = f"https://translate.google.com/translate_tts?ie=UTF-8&q={quote(text)}&tl={lang_code}&client=tw-ob"
                ...
                try:
                    ...
                    self.show_info("გახმოვანების შეცდომა")
        else:
            try:
                tts.speak(message=text)
            except Exception:
