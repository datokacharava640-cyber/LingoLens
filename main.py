...
# --- 1. ინტერნეტის ინდიკატორის ვიჯეტი ---
class NetworkIndicator(Widget):
    def set_status(self, status):
        self.canvas.before.clear()
        with self.canvas.before:
            if status == "green":
                Color(0.1, 0.8, 0.2, 1)  # მწვანე - კავშირი იდეალურია
            elif status == "yellow":
                Color(0.9, 0.7, 0.1, 1)  # ყვითელი - შეფერხება / ნელი კავშირი
            else:
                Color(0.9, 0.2, 0.2, 1)  # წითელი - ინტერნეტი არ არის
...
<MainScreen>:
    ...
    BoxLayout:
        ...
        BoxLayout:
            ...
            # ინტერნეტის სტატუსის ინდიკატორი (მწვანე/ყვითელი/წითელი)
            NetworkIndicator:
                id: net_indicator
                size_hint_x: None
                width: '24dp'
...
class MainScreen(Screen):
    ...
    def on_enter(self):
        # ყოველ 5 წამში ამოწმებს ინტერნეტის სტატუსს
    ...
    def check_network_status(self, dt):
        def _check():
            try:
                # ამოწმებს პინგს Vercel-ის API სერვერთან
