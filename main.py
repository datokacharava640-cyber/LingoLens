...
class AndroidKeyStoreBridge:
    ...
    def get_secure_key(alias="AZURE_API_KEY"):
        ...
        raw_b64 = "Nm5tR0NFQnV5NXdUM1huT3Z4d0RtQXl0OVJUSmYzb1d0MXYzSTdBZ3NkYlk2ZmhVUVIpNUpRUUo5OUNIQUNZZUJqRlgzdzNBQUFZQUNPR1dHZ1Q="
        try:
...
class LingoLensPro(MDApp):
    ...
    def open_conversation_dialog(self):
        box = MDBoxLayout(orientation='vertical', spacing="10dp", size_hint_y=None, height="180dp", padding="10dp")
        ...
        box.add_widget(btn1)
        box.add_widget(btn2)
        btn1.on_release = lambda: self.start_duplex_stream('target')
        btn2.on_release = lambda: self.start_duplex_stream('source')
    ...
    def open_favorites_dialog(self):
        ...
        MDDialog(title="რჩეულები", type="custom", content_cls=content).open()
    ...
    def open_history_dialog(self):
        ...
        MDDialog(title="ისტორია", type="custom", content_cls=content).open()
    ...
    def show_popup(self, title, text, is_confirm=False, text_to_trans=""):
        def confirm(x):
            if text_to_trans:
                self.root.ids.input_text.text = text_to_trans
                self.execute_realtime_translation(text_to_trans)
            if self.dialog:
                self.dialog.dismiss()

        btns = [MDRaisedButton(text="დიახ", font_name="Roboto", on_release=confirm)] if is_confirm else [MDRaisedButton(text="OK", font_name="Roboto", on_release=lambda x: self.dialog.dismiss())]
