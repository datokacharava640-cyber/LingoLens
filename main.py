...
# Render-ის თქვენი რეალური WSS მისამართი
...
class LingoLensApp(App):
    def build(self):
        ...
        # თუ ფონტი გაქვთ პროექტში: font_name="NotoSansGeorgian.ttf"
    ...
    def on_start(self):
        ...
        # WebSocket კავშირის გაშვება ცალკე ნაკადში
    ...
    def connect_to_proxy(self):
        def on_open(ws):
            ...
            # 1. Gemini-სთან დაკავშირება Handshake-ით
            setup_msg = {
                "setup": {
            ...
            # 2. მიკროფონის აუდიო ნაკადის გაშვება
                ...
                if "serverContent" in data:
                    ...
                    for part in parts:
                        if "inlineData" in part and part["inlineData"]["mimeType"].startswith("audio/pcm"):
                            ...
                            # აუდიოს მიღება წარმატებით მოხდა
            except Exception as e:
