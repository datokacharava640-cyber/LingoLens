...
class LingoLensApp(App):
    def build(self):
        ...
        # 2. Fifantenana fiteny + Hanova (SWAP)
        ...
        # 3. Fampidirana lahatsoratra sy fandikana
        self.input_text = TextInput(
            hint_text="Mampidira lahatsoratra...", 
            size_hint_y=0.25, 
            font_name=config.FONT_PATH,
            multiline=True
        )
        ...
        # 4. Bokotra fitaovana (Mihaino, Mandika/Kopia, Mamafa)
        tools_grid = GridLayout(cols=3, size_hint_y=0.08, spacing=5)
        
        btn_speak = Button(text="Mihaino", font_name=config.FONT_PATH, on_press=self.handle_tts)
        btn_copy = Button(text="Kopia", font_name=config.FONT_PATH, on_press=self.copy_output)
        btn_clear = Button(text="Mamafa", font_name=config.FONT_PATH, on_press=self.clear_all)
        ...
        # 5. Fizotran'ny rafitra (Status Bar)
        self.status_label = Label(
            text="Vready ny rafitra", 
            size_hint_y=0.05, 
            font_name=config.FONT_PATH,
            color=(0.8, 0.8, 0.8, 1)
        )
    ...
    # Fandikana ho azy rehefa manoratra (Debounce 0.8 segondra)
    ...
    def trigger_translation(self):
        ...
        self.status_label.text = "Am-pandikana..."
        
        # Torolalana ho an'ny Gemini mba handika tsara sy ho voajanahary
        prompt = (
            f"You are a professional translator. Translate the following text naturally, "
            f"idiomatically, and accurately from {self.src_lang} to {self.target_lang}. "
            f"Do NOT translate word-for-word if it sounds unnatural. "
            f"Output ONLY the final translation without quotes or extra text:\n\n{val}"
        )
    ...
    def update_output(self, text):
        self.output_text.text = text if text else "Misy fahadisoana amin'ny fandikana"
        self.status_label.text = "Vonona"

    # Fampandehanana ny feo (TTS)
    ...
    def copy_output(self, instance):
        if self.output_text.text:
            ...
            self.status_label.text = "Voa-kopia ny lahatsoratra!"

    def clear_all(self, instance):
        self.input_text.text = ""
        self.output_text.text = ""
        self.status_label.text = "Efa nadio"
    ...
    def run_academic(self):
        val = self.input_text.text.strip()
        if val:
            self.status_label.text = "Fandikana amin'ny fomba akademika..."
    ...
    def run_grammar(self):
        val = self.input_text.text.strip()
        if val:
            self.status_label.text = "Fanamarinana ny fitsipiky ny teny..."
