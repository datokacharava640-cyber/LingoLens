...
class LingoLensRealTimeUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        header_box.add_widget(CustomLabel(text='LingoLens Live AI', font_size='18sp', bold=True))
        
        btn_key = CustomButton(text='API Key', size_hint_x=0.25, background_color=(0.4, 0.4, 0.4, 1))
        btn_key.bind(on_press=self.open_api_key_popup)
        header_box.add_widget(btn_key)

        btn_about = CustomButton(text='About', size_hint_x=0.22, background_color=(0.3, 0.5, 0.7, 1))
    ...
    def open_about_popup(self, instance):
        ...
        content.add_widget(CustomLabel(text=info_text, font_size='14sp', halign='center'))
        
        btn_close = CustomButton(text="Close", size_hint_y=0.25)
    ...
    def start_speech_recognizer(self, lang_code):
        ...
        try:
            ...
            class ContinuousSpeechListener(PythonJavaClass):
                ...
                def __init__(self, callback, restart_callback, partial_callback=None):
                    super().__init__()
                ...
                def onPartialResults(self, results):
                    matches = results.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)
                    if matches and matches.size() > 0 and self.partial_callback:
                        self.partial_callback(matches.get(0))
            ...
            listener = ContinuousSpeechListener(
                ...
                self.on_partial_speech_recognized
            )
    ...
    def on_partial_speech_recognized(self, text):
        Clock.schedule_once(lambda dt: self._process_partial_input(text))

    def _process_partial_input(self, text):
        self.input_text.text = text
        if len(text.strip()) > 3 and text != self.last_translated_text:
            self.last_translated_text = text
            self.trigger_translation(auto_speak=False)
...
class LingoLensRealTimeApp(App):
    def build(self):
        global GEORGIAN_FONT
        # უსაფრთხოდ ვამოწმებთ მხოლოდ პროექტის შიგნით არსებულ NotoSansGeorgian.ttf ფაილს
        try:
    ...
    def on_start(self):
        # ეკრანის ჩართულობის შენარჩუნება უსაფრთხოდ ჩართვის შემდეგ
        try:
    ...
    def request_android_permissions(self, dt):
        try:
            ...
            self.request_battery_optimization_disable()
        except Exception as e:
            print(f"Permission Error: {e}")

    def request_battery_optimization_disable(self):
        try:
            from jnius import autoclass
            PythonActivity = autoclass('org.kivy.android.PythonActivity')
            Intent = autoclass('android.content.Intent')
