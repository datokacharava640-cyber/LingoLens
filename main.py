...
class LingoLensUI(BoxLayout):
    def __init__(self, user_data_dir=".", **kwargs):
        ...
        btn_camera = Button(text="[CAM] კამერა", size_hint_x=0.25, font_name=DEFAULT_FONT)
        btn_camera.bind(on_press=self.open_camera)

        btn_menu = Button(text="[=] მენიუ", size_hint_x=0.25, font_name=DEFAULT_FONT)
        ...
        # 5. Dual Conversation Buttons (კუბიკების გარეშე)
    ...
    def open_camera(self, instance):
        if platform == 'android':
            try:
                ...
                self.show_info(f"კამერის ჩართვის შეცდომა: {e}")
        else:
    ...
    def on_activity_result(self, request_code, result_code, intent):
        if request_code == 1001 and result_code == -1:
            ...
            self.show_info("სურათი გადაღებულია! OCR დამუშავება ჩაირთვება...")
