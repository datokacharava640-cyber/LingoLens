...
# კამერის როტაციისთვის საჭირო იმპორტები
from kivy.graphics.pushmatrix import PushMatrix
from kivy.graphics.popmatrix import PopMatrix
...
# Gemini API Configurations via Direct REST API
# ⚠️ ჩასვი შენი ნამდვილი Google Gemini API key ქვემოთ ბრჭყალებში!
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "ჩასვი_შენი_GEMINI_API_KEY_აქ")
...
def call_gemini_api(prompt, image_bytes=None):
    if not GEMINI_API_KEY or GEMINI_API_KEY == "ჩასვი_შენი_GEMINI_API_KEY_აქ":
        return "შეცდომა: GEMINI_API_KEY არ არის გაწერილი main.py-ში!"
    ...
    try:
        response = requests.post(GEMINI_URL, json=data, headers=headers, timeout=20)
...
class LingoLensApp(App):
    ...
    def build(self):
        ...
        btn_clear = Button(text="Clear", background_color=(0.7, 0.2, 0.2, 1), font_name=font_to_use)
    ...
    def init_camera(self):
        try:
            ...
            # 🔄 Android-ზე კამერის გასწორება (-90 გრადუსით შეტრიალება)
            if platform == 'android':
                with self.camera.canvas.before:
                    PushMatrix()
                    self.cam_rotate = Rotate(angle=-90, axis=(0, 0, 1))
                with self.camera.canvas.after:
                    PopMatrix()

                def update_cam_origin(instance, value):
                    self.cam_rotate.origin = instance.center

                self.camera.bind(pos=update_cam_origin, size=update_cam_origin)

            self.cam_box.add_widget(self.camera)
            self.status.text = "კამერა ჩართულია"
        except Exception as e:
            self.status.text = f"კამერა მიუწვდომელია: {e}"
    ...
    def show_history_popup(self, instance):
        ...
        btn_clear_db = Button(text="Clear All History", background_color=(0.8, 0.2, 0.2, 1), font_name="Roboto")
    ...
    def translate_task(self, text):
        ...
        def async_run():
            ...
            prompt = f"Translate text from language '{src_lang}' to language '{tgt_lang}'. Only provide the direct translation without additional formatting or explanations:\n{text}"
    ...
    def reset_mic_buttons(self):
        self.btn_mic_a.text = "[A] Speaker A"
        self.btn_mic_a.background_color = (0.1, 0.7, 0.4, 1)
        self.btn_mic_b.text = "[B] Speaker B"
    ...
    def capture_frame(self, dt):
        ...
        try:
            ...
            def async_ocr():
                try:
                    prompt = f"Extract all visible text from this image and translate it to target language '{self.tgt_spin.text}'. Return result strictly in format 'EXTRACTED_TEXT ||| TRANSLATED_TEXT'"
