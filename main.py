...
class CustomSpinnerOption(SpinnerOption):
    def __init__(self, **kwargs):
        ...
        self.background_color = get_color_from_hex('#2A2D3A')
        self.color = get_color_from_hex('#FFFFFF')

# მსოფლიოს ძირითადი ენები
LANGUAGES = {
    ...
    "🇯🇵 日本語": {"code": "ja", "stt": "ja-JP"},
    "🇰🇷 한국어": {"code": "ko", "stt": "ko-KR"},
    "🇸🇦 العربية": {"code": "ar", "stt": "ar-SA"},
    "🇮🇳 हिन्दी": {"code": "hi", "stt": "hi-IN"},
    "🇵🇹 Português": {"code": "pt", "stt": "pt-PT"},
    "🇺🇦 Українська": {"code": "uk", "stt": "uk-UA"},
    "🇵🇱 Polski": {"code": "pl", "stt": "pl-PL"},
    "🇳🇱 Nederlands": {"code": "nl", "stt": "nl-NL"},
    "🇬🇷 Ελληνικά": {"code": "el", "stt": "el-GR"},
    "🇦🇿 Azərbaycan": {"code": "az", "stt": "az-AZ"},
    "🇦🇲 Հայերեն": {"code": "hy", "stt": "hy-AM"}
}
...
class LingoLensUI(BoxLayout):
    def __init__(self, user_data_dir=".", **kwargs):
        ...
        # 1. Header (კამერის ხატულა, Title, Overlay, Menu)
        header = BoxLayout(size_hint_y=0.08, spacing=6)
        ...
        # 2. Foreigner Language Dropdown
        foreign_box = BoxLayout(size_hint_y=0.08, spacing=6)
        lbl_foreign = Label(
            text="🌐 უცხოელის ენა:", 
            font_name=DEFAULT_FONT, 
            size_hint_x=0.4,
            color=get_color_from_hex('#A0A5BD')
        )
        
        foreign_options = [k for k in LANGUAGES.keys() if "ქართული" not in k]
        ...
        input_container = BoxLayout(size_hint_y=0.28, spacing=6)
        self.input_text = TextInput(
            hint_text="✍️ მოსმენილი / შეყვანილი ტექსტი...", 
            font_name=DEFAULT_FONT,
            size_hint_x=0.78,
            background_normal='',
            background_color=get_color_from_hex('#1F222E'),
        ...
        # 5. Dual Conversation Buttons (კომპაქტური და დაპატარავებული)
    ...
    def open_menu(self, instance):
        p = Popup(title="⚙️ მენიუ", size_hint=(0.8, 0.45), title_font=DEFAULT_FONT)
    ...
    def open_history(self):
        ...
        if not items:
            ...
            for s, t in items:
                lbl = Label(text=f"• {s} ➡️ {t}", font_name=DEFAULT_FONT, size_hint_y=None, height=40, halign='left', valign='middle')
    ...
    def show_info(self, text):
        p = Popup(title="ℹ️ შეტყობინება", size_hint=(0.7, 0.3), title_font=DEFAULT_FONT)
