...
# ---------------------------------------------------------
# ქართული ფონტის გლობალური რეგისტრაცია
try:
...
class LingoLensUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        btn_hist = Button(text="History", size_hint_x=0.3, font_name=DEFAULT_FONT)
        ...
        self.src_spinner = Spinner(text="ქართული", values=list(LANGUAGES.keys()), size_hint_x=0.4, font_name=DEFAULT_FONT)
        self.target_spinner = Spinner(text="English", values=list(LANGUAGES.keys()), size_hint_x=0.4, font_name=DEFAULT_FONT)
    ...
    def open_history(self, instance):
        p = Popup(title="History", size_hint=(0.8, 0.8), title_font=DEFAULT_FONT)
        box = BoxLayout(orientation='vertical')
        for s, t in self.db.get():
            box.add_widget(Label(text=f"{s} -> {t}", font_name=DEFAULT_FONT))
        p.content = box
        p.open()
