...
# Android Text-To-Speech ინტეგრაცია pyjnius-ით
TTS = None
if platform == 'android':
    try:
...
# ფონტის შემოწმება (ჩაანაცვლეთ თქვენი ფონტის სახელით საჭიროებისამებრ)
FONT_PATH = "NotoSansGeorgian-Regular.ttf" if os.path.exists("NotoSansGeorgian-Regular.ttf") else ("DejaVuSans.ttf" if os.path.exists("DejaVuSans.ttf") else "Roboto")
    ...
    def speak_text(self, text, lang_code):
        ...
        if platform == 'android' and TTS:
            try:
                ...
                TTS.setLanguage(loc)
                TTS.speak(text_to_speak, TextToSpeech.QUEUE_FLUSH, None, None)
            except Exception as e:
