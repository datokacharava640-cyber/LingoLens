...
# ენები გადაყვანილია ლათინურ/ინგლისურ დასახელებებზე კუბიკების თავიდან ასაცილებლად
LANGUAGES = {
    "Georgian": "ka",
    "English (US)": "en_US",
    "English (UK)": "en_GB",
...
class MainScreen(Screen):
    ...
    def _delayed_translate(self, text, callback_after=None):
        ...
        def _on_result(success, response):
            if success and isinstance(response, dict):
                ...
                self.ids.output_text.text = "[კავშირი ვერ დამყარდა]"
    ...
    def start_speech_to_text(self, lang_code="ka", on_complete_callback=None):
        if platform == 'android':
            try:
                ...
                intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE, lang_code.replace('_', '-'))
    ...
    def speak_output_text(self, custom_text=None):
        ...
        if "ka" in self.target_lang:
            ...
            try:
                ...
                def _speak():
                    tts_obj = TextToSpeech(AndroidPythonActivity, None)
                    tts_obj.setLanguage(Locale.US)
                    tts_obj.speak(text, TextToSpeech.QUEUE_FLUSH, None, None)
    ...
    def _play_georgian_audio(self, text):
        try:
            cache_dir = os.path.join(App.get_running_app().user_data_dir, "audio_cache")
            os.makedirs(cache_dir, exist_ok=True)
            filename = os.path.join(cache_dir, f"{abs(hash(text))}.mp3")
    ...
    def swap_languages(self):
        # 1. ენების კოდებისა და ღილაკის ტექსტების გაცვლა
        ...
        # 2. ტექსტური ველების (შესყვანი და ნათარგმნი ტექსტების) ადგილების გაცვლა
