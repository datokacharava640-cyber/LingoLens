...
APP_CREDITS_TEXT = (
    f"LingoLens v{CURRENT_VERSION} | ავტორი: Dato Kacharava\n"
    "Powered by Google & Gemini APIs | Licensed under EISI / Open License"
)
...
# ---------------------------------------------------------
# 2. მსოფლიოს ენების სრული სია (100+ ენა)
# ---------------------------------------------------------
LANGUAGES = {
    "ავტოდაჭერა": "auto",
    "ქართული": "ka",
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # ავტომატური განახლების შემოწმება ჩართვისას
        Clock.schedule_once(lambda dt: self.check_for_updates(silent=True), 2)
    ...
    def _build_ui(self):
        ...
        # Footer Label (ავტორის, Google/Gemini და EISI ლიცენზიის მითითება)
        footer_kwargs = {
    ...
    # ---------------------------------------------------------
    # 7. ავტომატური განახლების შემოწმება (Auto Update Check)
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 8. Android Speech-To-Text (STT) Result Handler
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 10. ენების არჩევა და ძებნა (100+ ენა)
    # ---------------------------------------------------------
    def open_language_selector(self, mode):
        trigger_vibration()
        main_layout = BoxLayout(orientation="vertical", padding=10, spacing=8)
        search_input = TextInput(
            hint_text="ძებნა (მაგ: ქართული, English, es...)",
            size_hint_y=None,
            height="40dp",
            multiline=False
        )
    ...
    # ---------------------------------------------------------
    # 11. დამხმარე ფუნქციები (TTS, Clear, Copy, Save)
    # ---------------------------------------------------------
    def speak_output_text(self):
        ...
        if platform == "android":
            try:
                ...
                tts = TextToSpeech(PythonActivity.mActivity, on_tts_init)
            except Exception as e:
                print(f"TTS Error: {e}")
        else:
    ...
    # ---------------------------------------------------------
    # 12. Extra Hubs (Interpreter, Dictionary, Quiz, File)
    # ---------------------------------------------------------
