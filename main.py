...
class GlassPanel(BoxLayout):
  ...
  def __init__(self, bg_color=(0.1, 0.1, 0.18, 0.75), border_color=(0.3, 0.5, 0.9, 0.6), radius=[16], **kwargs):
  ...
  def update_canvas(self, *args):
    ...
    with self.canvas.before:
      ...
      Line(rounded_rectangle=(self.x, self.y, self.width, self.height, self.radius[0]), width=1.2)
...
class LingoLensUltraApp(App):
  ...
  def build(self):
    ...
    self.lang_spinner = Spinner(
        ...
        values=('Auto Bidirectional', 'Georgian', 'English', 'German', 'French'),
        size_hint=(0.38, 1),
        background_normal='',
    ...
    # --- 2. Camera Viewport & Stereoscopic HUD Split ---
    ...
    # --- 3. Live Audio Waveform & AR Subtitles Glass Panel ---
    ...
    self.ar_label = Label(
        text='[color=00ffff]✨ LingoLens Glass UI Ready[/color]\n[color=888888]Tap screen to focus ROI & Mute[/color]',
        font_size='15sp',
        markup=True,
        halign='center',
        valign='middle',
    ...
    # Initialize Threads & Loops
    threading.Thread(target=self.websocket_thread, daemon=True).start()
    Clock.schedule_interval(self.process_and_stream_frame, self.stream_fps_interval)

    return self.main_layout

  # --- 1. Dynamic Android Permissions Request ---
  def on_start(self):
    """ითხოვს კამერისა და მიკროფონის უფლებებს აპლიკაციის ჩართვისთანავე"""
    if platform == 'android':
  ...
  def open_settings(self, instance):
    ...
    popup = Popup(
        title='API Settings', content=content, size_hint=(0.85, 0.4)
    )
