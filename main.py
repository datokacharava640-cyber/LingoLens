...
Version             : 3.6.5 (Camera Toggle Control)
...
# SSL Certificate setup for Android
try:
    import certifi
...
class LingoLensApp(App):
    ...
    def build(self):
        ...
        self.root_layout.add_widget(Label(
            text="LingoLens AI - Live OCR & Translate",
            font_size='16sp',
            bold=True,
            size_hint_y=0.06,
            color=(0.2, 0.8, 0.5, 1),
        ...
        # ენების არჩევა / Swap
        lang_layout = BoxLayout(orientation='horizontal', size_hint_y=0.07, spacing=5)
        ...
        # კამერის კონტეინერი
        self.cam_box = BoxLayout(size_hint_y=0.28)
        ...
        self.btn_live = Button(
            text="Live OCR", background_color=(0.1, 0.6, 0.9, 1),
            font_name=font_to_use
        )
        ...
        btn_speak = Button(
            text="Read Text", background_color=(0.8, 0.4, 0.1, 1),
            font_name=font_to_use
        )
        ...
        btn_copy = Button(
            text="Copy", background_color=(0.2, 0.7, 0.3, 1),
            font_name=font_to_use
        )
        ...
        # Status Bar
        self.status = Label(
            text="მზადაა მუშაობისთვის", size_hint_y=0.05, font_size='12sp',
            font_name=font_to_use
        )
            ...
            try:
                ...
                def callback(permissions, results):
                    if all(results):
                        ...
                        self.update_status("კამერის ნებართვა უარყოფილია")
    ...
    def init_camera(self):
        try:
            ...
            # ნაგულისხმევად კამერა გამორთულია (play=False)
            self.camera = Camera(play=False, resolution=(640, 480))
            self.cam_box.add_widget(self.camera)
            self.status.text = "კამერა მზადაა (გამორთულია)"
        except Exception as e:
            print(f"[Camera Init Error]: {e}")
            self.cam_box.clear_widgets()
            self.cam_box.add_widget(Label(text="კამერა მიუწვდომელია", font_name="Roboto"))
            self.status.text = "კამერის ინიციალიზაცია ვერ მოხერხდა"
