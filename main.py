...
class LingoLensDesktopApp(App):
    def build(self):
        # 1.5 წამის დაყოვნება, რომ აპლიკაცია ჯერ სრულად ჩაიტვირთოს და არ დაიხუროს
        Clock.schedule_once(lambda dt: self.request_android_permissions(), 1.5)
    ...
    # 3. კამერა / OCR (რეალური Android გალერეის ინტეგრაციით)
                    ...
                    try:
                        ...
                        def on_activity_result(request_code, result_code, intent_data):
                            if request_code == 1001 and result_code == -1 and intent_data:
                                ...
                                if uri:
                                    # Native URI-დან ფაილის რეალური path-ის მიღება
