# ==============================================================================
# LingoLens Ultra Pro Live AI 🇬🇪 - Auto-Updating & Self-Evolving Edition
# Creator / Author: David Kacharava (დავით კაჭარავა)
# Country: Georgia 🇬🇪 | Date: აგვისტო 22, 2026
# ==============================================================================
...
APP_VERSION = "2.0.0"
GITHUB_REPO_OWNER = "DavidKacharava"  # შეცვალეთ თქვენი GitHub მომხმარებლით
GITHUB_REPO_NAME = "LingoLens-Ultra-Pro"

# Remote AI Config - სერვერიდან დინამიურად განახლებადი პარამეტრები
AI_CONFIG = {
    "system_prompt": "You are an expert AI translator.",
    "debouncing_rate": 0.3
}
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.source_lang = "ka"
        self.target_lang = "en"
        Clock.schedule_once(lambda dt: self.fetch_remote_ai_config(), 1)
        Clock.schedule_once(lambda dt: self.check_for_updates(manual=False), 3)

    def fetch_remote_ai_config(self):
        # დინამიური AI კონფიგურაციის წამოღება GitHub Raw-დან
        url = f"https://raw.githubusercontent.com/{GITHUB_REPO_OWNER}/{GITHUB_REPO_NAME}/main/ai_config.json"
        UrlRequest(url, on_success=self._on_config_success, timeout=4)

    def _on_config_success(self, req, res):
        try:
            if isinstance(res, dict):
                AI_CONFIG.update(res)
        except Exception:
            pass
    ...
    def _on_update_check_fail(self, manual):
        if manual:
            self.show_info_popup("შეცდომა", "სერვერთან კავშირი ვერ დამყარდა.")

    def show_update_popup(self, version, url):
        box = BoxLayout(orientation='vertical', padding=10, spacing=10)
        box.add_widget(Label(
            text=f"ხელმისაწვდომია ახალი ვერსია: v{version}\nგსურთ ავტომატური ჩამოტვირთვა?",
            font_name=FONT_PATH, font_size='14sp', halign='center'
        ))
    ...
    def on_live_translate(self, text):
        ...
        rate = AI_CONFIG.get("debouncing_rate", 0.3)
        Clock.schedule_once(lambda dt: self._delayed_translate(cleaned), rate)

    def _delayed_translate(self, text):
        url = f"https://translate.googleapis.com/translate_a/single?client=gtx&sl={self.source_lang}&tl={self.target_lang}&dt=t&dt=bd&dt=qc&q={quote(text)}"
        headers = {'User-Agent': 'Mozilla/5.0'}
        UrlRequest(
            url, req_headers=headers,
            on_success=lambda req, res: self._on_translation_success(res),
            on_error=lambda req, err: self._on_translation_error(),
            timeout=5
        )
    ...
    def _on_translation_error(self):
        self.ids.output_text.text = "კავშირის ხარვეზი. შეამოწმეთ ინტერნეტი..."

    def open_live_dialog_mode(self):
        box = BoxLayout(orientation='vertical', padding=10, spacing=8)
        dialog_log = TextInput(hint_text="ორმხრივი ცოცხალი დიალოგი...", font_name=FONT_PATH,
                               readonly=True, background_color=(0.08, 0.1, 0.13, 1),
                               foreground_color=(1, 1, 1, 1), size_hint_y=0.8)
        box.add_widget(dialog_log)
        ...
        btn_user1.bind(on_release=lambda x: self.start_speech_recognition('source'))
        btn_user2.bind(on_release=lambda x: self.start_speech_recognition('target'))
    ...
    def speak_text(self, text, lang_code):
        ...
        if platform == 'android' and TTS:
            try:
                ...
                TTS.speak(text.strip(), TextToSpeech.QUEUE_FLUSH, None, None)
