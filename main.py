...
class LingoLensDesktopApp(App):
    ...
    # 1. FAʻALILIUGA MA LE FILIFILIGA O GAGANA (Translator with Language Selector)
    def open_translator(self, instance):
        ...
        btn_listen = Button(text="მოსმენა", font_name=font, on_press=lambda x: speak_text(out.text) if out.text.strip() else None)
        ...
        def run_trans_async():
            ...
            if text_to_translate:
                ...
                try:
                    ...
                    Clock.schedule_once(lambda dt: setattr(status_lbl, 'text', 'API შეცდომა!'))

        btn_trans.bind(on_press=lambda x: threading.Thread(target=run_trans_async, daemon=True).start())
        ...
        def run_grammar_async():
            ...
            if text_to_check:
                ...
                try:
                    ...
                    print("Grammar API error:", e)
                    Clock.schedule_once(lambda dt: setattr(status_lbl, 'text', 'API შეცდომა!'))

        btn_check.bind(on_press=lambda x: threading.Thread(target=run_grammar_async, daemon=True).start())
    ...
    # 2. KAMERA MA LE OCR (Camera & Image Text Recognition)
    ...
    def on_gallery_select(self, selection):
        if selection and len(selection) > 0:
            ...
            # Faʻatinoina o le faitauina o tusitusiga i ata (OCR logic)
            try:
                import pytesseract
    ...
    # 3. FAʻASAOINA O LE API KEY I JSON (Persistent Settings)
