...
# ქართული შრიფტის გლობალური ჩატვირთვა მთელი აპლიკაციისთვის
...
class LingoLensUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        btn_ai = Button(text='🧠 AI თარგმნა', background_color=(0.1, 0.5, 0.9, 1))
        btn_ai.bind(on_press=self.start_translation)
        grid.add_widget(btn_ai)

        btn_voice = Button(text='🎙️ ხმოვანი რეჟიმი', background_color=(0.1, 0.7, 0.3, 1))
        btn_voice.bind(on_press=self.voice_action)
        grid.add_widget(btn_voice)

        btn_cam = Button(text='📷 კამერა / OCR', background_color=(0.9, 0.4, 0.1, 1))
        btn_cam.bind(on_press=self.camera_action)
        grid.add_widget(btn_cam)

        btn_mode = Button(text='🪟 Overlay რეჟიმი', background_color=(0.6, 0.2, 0.8, 1))
