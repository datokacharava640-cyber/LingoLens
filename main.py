...
if platform == 'android':
    ...
    # AudioFX ხმაურის და ეხოს ჩასახშობად
...
class LingoLensApp(App):
    ...
    def start_foreground_service(self):
        """5. Android Foreground Service ჩაკეცვისას მუშაობისთვის"""
        try:
            activity = PythonActivity.mActivity
            Intent = autoclass('android.content.Intent')
            # სერვისის გაშვება ფონური პროცესისთვის
    ...
    def on_language_change(self, spinner, text):
        """4. ენის დინამიური ცვლილება - ავტომატური Re-connect"""
        if "CUSTOM" in text:
    ...
    def _is_speech_present(self, raw_bytes, threshold=300):
        if len(raw_bytes) < 2:
            return False
    ...
    def stream_audio(self, ws):
        """2. AEC (Acoustic Echo Cancellation) + Noise Suppression & AGC & VAD"""
        if platform == 'android':
            try:
                sample_rate = 16000
    ...
    def _capture_and_send_frame(self):
        """3. კადრის 90 გრადუსით შემოტრიალება სწორი პორტრეტისთვის"""
        try:
            if hasattr(self, 'camera_widget') and self.camera_widget and self.camera_widget.texture:
                ...
                # კადრის შემოტრიალება სწორი პორტრეტული მხარისთვის
