...
# --- 1. ყველა შექმნილი მოდულის იმპორტი ---
...
class LingoLensApp(App):
    def build(self):
        ...
        # 1. Header (სათაური + ქართული დროშის ანიმაციის იმიტაცია)
        header = Label(
        ...
        # 2. Status Bar (სერვერის სტატუსი + სთრიქები)
        self.status_label = Label(
            text=f"⚡ Waking up AI Engine... | {self.streak_tracker.get_status()}",
            font_size='12sp',
            size_hint_y=0.07,
            color=(0.2, 0.8, 0.2, 1)
        )
        ...
        self.input_field = TextInput(
            hint_text="Enter text, phrase, or say 'LingoLens'...",
            multiline=True,
            size_hint_y=0.20
        )
        ...
        self.output_label = Label(
            text="[AI Translation & Live Module Output Will Appear Here]",
            markup=True,
            size_hint_y=0.25
        )
        root.add_widget(self.output_label)

        # 5. Control Panel Grid (ყველა ფუნქციის მენეჯერი)
        ...
        btn_specs = [
            ...
            ("🗣️ AI Voice Clone", self.feat_voice_clone, (0.9, 0.3, 0.5, 1)),
            ("🔥 Daily Streaks", self.feat_check_streak, (0.9, 0.5, 0.1, 1)),
            ("🎁 Invite & Rewards", self.feat_referral, (0.2, 0.8, 0.4, 1)),
            ("👁️ AR Camera View", self.feat_ar_camera, (0.9, 0.4, 0.2, 1)),
        ...
        # 6. ფონური პროცესები (Cold-Start Ping & Wake Word Check)
        Clock.schedule_once(self.ping_server, 0.5)
        Clock.schedule_interval(self.check_wake_word, 2.0)

        return root

    # --- Cold-Start Ping (Render-ის გაღვიძება ჩართვისთანავე) ---
    def ping_server(self, dt):
        def _ping():
            try:
                ...
                Clock.schedule_once(lambda d: setattr(self.status_label, 'text', f"🟡 Offline / Standby | {self.streak_tracker.get_status()}"), 0)
        threading.Thread(target=_ping, daemon=True).start()

    # --- Wake Word Background Check ---
    def check_wake_word(self, dt):
        ...
        if is_triggered:
            ...
            self.output_label.text = "⚡ Wake Word 'LingoLens' Detected!"
