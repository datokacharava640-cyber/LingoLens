...
class LingoLensUI(BoxLayout):
    def __init__(self, user_data_dir=".", **kwargs):
        ...
        # 1. Header (მენიუ, კამერა და Floating Mode)
        header = BoxLayout(size_hint_y=0.08, spacing=5)
        title = Label(text="LingoLens Pro", font_size='16sp', bold=True, font_name=DEFAULT_FONT)
        
        btn_float = Button(text="[+] Overlay", size_hint_x=0.25, font_name=DEFAULT_FONT, font_size='11sp')
        btn_float.bind(on_press=self.request_overlay_permission)

        btn_camera = Button(text="[CAM]", size_hint_x=0.2, font_name=DEFAULT_FONT, font_size='11sp')
        btn_camera.bind(on_press=self.open_camera)

        btn_menu = Button(text="[=]", size_hint_x=0.18, font_name=DEFAULT_FONT, font_size='11sp')
        ...
        if platform == 'android':
            try:
                ...
                if not Settings.canDrawOverlays(activity):
                    ...
                    self.show_info("Overlay ნებართვა უკვე ჩართულია!")
            except Exception as e:
                self.show_info(f"Overlay შეცდომა: {e}")
        else:
            self.show_info("Overlay მუშაობს მხოლოდ Android-ზე")
    ...
    def on_activity_result(self, request_code, result_code, intent):
        if request_code == 1001 and result_code == -1:
            ...
            self.show_info("სურათი გადაღებულია!")
