...
# უსაფრთხოდ იმპორტირება config.py-დან
try:
...
class MainScreen(Screen):
    ...
    def process_image_with_gemini(self, image_path):
        ...
        self.output_text.text = "მიმდინარეობს ფოტოს დამუშავება..."
        ...
        def _thread_target():
            try:
                ...
                prompt = (
                    f"1. Extract all text from this image as accurately as possible.\n"
                    f"2. Translate that text into {target_lang_name}.\n"
                    f"Format the output strictly as JSON with keys: \"original_text\" and \"translated_text\"."
                )
                ...
                with urllib.request.urlopen(req, timeout=15) as response:
                    if response.status == 200:
                        ...
                        Clock.schedule_once(
                            lambda dt: self._set_output_text("Gemini API-ს შეცდომა.")
                        )
            except Exception as e:
                Clock.schedule_once(
    ...
    def _on_activity_result(self, request_code, result_code, data):
        if request_code == 1002 and result_code == -1 and data is not None:
            ...
            if hasattr(self, 'photo_path') and os.path.exists(self.photo_path):
                self.process_image_with_gemini(self.photo_path)
