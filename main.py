...
# ფონტის უსაფრთხო ჩატვირთვის ლოგიკა
FONT_NAME = None

def setup_font():
    global FONT_NAME
    
    # 1. ამოწმებს APK-ში ჩაშენებულ font.ttf-ს (უნდა იყოს > 20KB)
    local_font = 'font.ttf'
    if os.path.exists(local_font) and os.path.getsize(local_font) > 20000:
        try:
            LabelBase.register(name='GeorgianFont', fn_regular=local_font)
            FONT_NAME = 'GeorgianFont'
            return
        except Exception as e:
            print(f"Local font failed: {e}")

    # 2. თუ ფაილი დაზიანებულია, იყენებს Android-ის შიდა სისტემურ ქართულ ფონტს
    android_fonts = [
        '/system/fonts/NotoSansGeorgian-Regular.ttf',
        '/system/fonts/NotoSansGeorgianUI-Regular.ttf'
    ]
    for sys_font in android_fonts:
        if os.path.exists(sys_font):
            try:
                LabelBase.register(name='GeorgianSysFont', fn_regular=sys_font)
                FONT_NAME = 'GeorgianSysFont'
                return
            except Exception as e:
                print(f"System font failed: {e}")
