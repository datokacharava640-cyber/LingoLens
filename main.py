...
class LingoLensPro(MDApp):
    ...
    def start_duplex_stream(self, mode):
        stt_code = LANGUAGES.get(self.root.ids.btn_src.text, {}).get("stt", "ka-GE") if mode == 'source' else LANGUAGES.get(self.root.ids.btn_trg.text, {}).get("stt", "en-US")
        if platform == 'android':
            try:
                from jnius import autoclass
                PythonActivity = autoclass('org.kivy.android.PythonActivity')
    ...
    def toggle_fav_current(self):
        ...
        if src and trg:
            ...
            self.show_popup("ფავორიტები", "სტატუსი განახლდა!")
    ...
    def show_popup(self, title, text, is_confirm=False, text_to_trans=""):
        if is_confirm:
            buttons = [
                MDRaisedButton(text="არა", on_release=lambda x: self.dialog.dismiss()),
                MDRaisedButton(text="დიახ", on_release=lambda x: (setattr(self.root.ids.input_text, 'text', text_to_trans), self.dialog.dismiss()))
            ]
        else:
