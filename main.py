...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.interpreter_active_side = None
        self.chat_box = None
    ...
    def _build_ui(self):
        ...
        btn_tts.bind(on_release=lambda x: self.speak_output_text(self.output_text.text, self.target_lang))
    ...
    # ---------------------------------------------------------
    # 7. ინტერაქტიული ორმხრივი თარჯიმანი (Live Chat)
    # ---------------------------------------------------------
    def open_interpreter_mode(self):
        ...
        # მართვის ღილაკები
        btn_layout = BoxLayout(size_hint_y=None, height="50dp", spacing=8)
        
        btn_style = {
            "background_normal": "",
            "background_color": (0.38, 0.22, 0.58, 1),
            "color": (1, 1, 1, 1),
            "bold": True
        }

        btn_src_name = self.btn_src.text if self.source_lang != "auto" else "თქვენ"
        btn_tgt_name = self.btn_tgt.text

        btn_speaker_src = Button(text=f"🎤 {btn_src_name}", **btn_style)
        if FONT_PATH: btn_speaker_src.font_name = FONT_PATH
        btn_speaker_src.bind(on_release=lambda x: self._start_interpreter_speech("src"))

        btn_speaker_tgt = Button(text=f"🎤 {btn_tgt_name}", **btn_style)
        if FONT_PATH: btn_speaker_tgt.font_name = FONT_PATH
        btn_speaker_tgt.bind(on_release=lambda x: self._start_interpreter_speech("tgt"))

        btn_layout.add_widget(btn_speaker_src)
        btn_layout.add_widget(btn_speaker_tgt)

        main_layout.add_widget(scroll)
        main_layout.add_widget(btn_layout)

        FontPopup(title="ორმხრივი თარჯიმანი (Live Chat)", content=main_layout, size_hint=(0.95, 0.9)).open()

    def _start_interpreter_speech(self, side):
        self.interpreter_active_side = side
        lang_code = self.source_lang if side == "src" else self.target_lang
        self.start_speech_to_text(lang_code)

    def handle_interpreter_speech_result(self, text):
        if not text or not self.interpreter_active_side:
            return

        if self.interpreter_active_side == "src":
            sl = self.source_lang if self.source_lang != "auto" else "ka"
            tl = self.target_lang
            speaker_name = self.btn_src.text
        else:
            sl = self.target_lang
            tl = self.source_lang if self.source_lang != "auto" else "ka"
            speaker_name = self.btn_tgt.text

        def _thread_target():
            try:
                ctx = ssl._create_unverified_context()
                query_encoded = urllib.parse.quote(text)
                url = f"https://translate.googleapis.com/translate_a/single?client=gtx&sl={sl}&tl={tl}&dt=t&q={query_encoded}"
                ...
                with urllib.request.urlopen(req, timeout=5, context=ctx) as response:
                    if response.status == 200:
                        ...
                        Clock.schedule_once(
                            lambda dt: self._add_chat_bubble(speaker_name, text, translated, tl)
                        )
            except Exception as e:
    ...
    def _add_chat_bubble(self, speaker, original, translated, target_lang):
        if not self.chat_box:
            return

        bubble = BoxLayout(orientation="vertical", size_hint_y=None, padding=8, spacing=4)
        bubble.bind(minimum_height=bubble.setter("height"))

        lbl_orig_kwargs = {
            "text": f"🗣 {speaker}: {original}",
            "color": (0.8, 0.7, 0.9, 1),
            "size_hint_y": None,
            "font_size": "14sp"
        }
        if FONT_PATH: lbl_orig_kwargs["font_name"] = FONT_PATH
        lbl_orig = Label(**lbl_orig_kwargs)
        lbl_orig.bind(texture_size=lambda instance, val: setattr(instance, 'height', val[1]))
        ...
        if FONT_PATH: lbl_trans_kwargs["font_name"] = FONT_PATH
        lbl_trans = Label(**lbl_trans_kwargs)
        lbl_trans.bind(texture_size=lambda instance, val: setattr(instance, 'height', val[1]))

        bubble.add_widget(lbl_orig)
        bubble.add_widget(lbl_trans)
        self.chat_box.add_widget(bubble)

        # ავტომატური წაკითხვა
        self.speak_output_text(translated, target_lang)

    # ---------------------------------------------------------
    # 8. კამერა, Gemini AI, STT, TTS
    # ---------------------------------------------------------
        ...
        if platform == "android":
            try:
                ...
                try:
                    ...
                    file_uri = FileProvider.getUriForFile(context, context.getPackageName() + ".fileprovider", photo_file)
                except Exception:
    ...
    def _show_image_path_dialog(self):
        ...
        inp_path = TextInput(hint_text="შეიყვანეთ ფოტოს გზა...", size_hint_y=None, height="40dp", multiline=False, background_color=(0.2, 0.12, 0.3, 1), foreground_color=(1, 1, 1, 1))
        if FONT_PATH: inp_path.font_name = FONT_PATH

        btn_process = Button(text="Gemini AI-ით ამოცნობა", size_hint_y=None, height="45dp", background_normal="", background_color=(0.45, 0.20, 0.65, 1), color=(1, 1, 1, 1))
        if FONT_PATH: btn_process.font_name = FONT_PATH
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=20, context=ctx) as response:
                    if response.status == 200:
                        ...
                        Clock.schedule_once(lambda dt: self._update_ui_after_gemini(orig_text, trans_text))
                    else:
                        Clock.schedule_once(lambda dt: self._set_output_text("Gemini API შეცდომა"))
            except Exception as e:
                Clock.schedule_once(lambda dt: self._set_output_text(f"ფოტოს ამოცნობა ვერ მოხერხდა: {e}"))
    ...
    def on_live_translate(self, text):
        ...
        self._debounce_event = Clock.schedule_once(lambda dt: self._execute_translation(text), 0.4)
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=5, context=ctx) as response:
                    if response.status == 200:
                        ...
                        data = json.loads(raw_data)
                        translated = "".join([item[0] for item in data[0] if item and item[0]])
                        Clock.schedule_once(lambda dt: self._handle_translation_success(text, translated))
            except Exception:
                Clock.schedule_once(lambda dt: self._set_output_text("ინტერნეტთან კავშირი არ არის / ქეში ვერ მოიძებნა"))
    ...
    def open_language_selector(self, mode):
        trigger_vibration()
        main_layout = BoxLayout(orientation="vertical", padding=10, spacing=8)
        search_input = TextInput(hint_text="ძებნა...", size_hint_y=None, height="42dp", multiline=False, background_color=(0.2, 0.12, 0.3, 1), foreground_color=(1, 1, 1, 1))
        ...
        def populate_languages(filter_text=""):
            ...
            for name, code in LANGUAGES.items():
                ...
                if filter_text.lower() in name.lower() or filter_text.lower() in code.lower():
                    btn = Button(text=f"{name} ({code})", size_hint_y=None, height="42dp", background_normal="", background_color=(0.28, 0.17, 0.42, 1), color=(0.95, 0.9, 1, 1))
                    if FONT_PATH: btn.font_name = FONT_PATH
                    btn.bind(on_release=lambda instance, c=code, n=name: self._select_language(mode, c, n, popup))
                    box.add_widget(btn)
    ...
    def speak_output_text(self, text=None, lang_code=None):
        trigger_vibration()
        text_to_speak = text if text else self.output_text.text.strip()
        code = lang_code if lang_code else self.target_lang
        ...
        if platform == "android":
            try:
                ...
                locale_obj = Locale("ka", "GE") if code == "ka" else Locale(code)

                def on_init(status):
                    if status == TextToSpeech.SUCCESS:
                        tts.setLanguage(locale_obj)
                        tts.speak(text_to_speak, TextToSpeech.QUEUE_FLUSH, None, "TTS_ID")
    ...
    def open_quiz_mode(self):
        ...
        input_ans = TextInput(hint_text="ჩაწერთ თარგმანი...", size_hint_y=None, height="40dp", multiline=False, background_color=(0.2, 0.12, 0.3, 1), foreground_color=(1, 1, 1, 1))
        if FONT_PATH: input_ans.font_name = FONT_PATH

        btn_check = Button(text="შემოწმება", size_hint_y=None, height="45dp", background_normal="", background_color=(0.45, 0.20, 0.65, 1), color=(1, 1, 1, 1))
        if FONT_PATH: btn_check.font_name = FONT_PATH
    ...
    def open_file_translator(self):
        trigger_vibration()
        layout = BoxLayout(orientation="vertical", padding=10, spacing=8)
        input_path = TextInput(hint_text="შეიყვანეთ .txt ფაილის სრული გზა...", size_hint_y=None, height="40dp", multiline=False, background_color=(0.2, 0.12, 0.3, 1), foreground_color=(1, 1, 1, 1))
        if FONT_PATH: input_path.font_name = FONT_PATH

        btn_load = Button(text="ფაილის წაკითხვა", size_hint_y=None, height="45dp", background_normal="", background_color=(0.45, 0.20, 0.65, 1), color=(1, 1, 1, 1))
        if FONT_PATH: btn_load.font_name = FONT_PATH
                ...
                with urllib.request.urlopen(req, timeout=4, context=ctx) as resp:
                    if resp.status == 200:
                        ...
                        if latest_ver != CURRENT_VERSION:
                            Clock.schedule_once(lambda dt: self._show_simple_popup("განახლება", f"ხელმისაწვდომია ახალი ვერსია v{latest_ver}!"))
            except Exception:
                pass
    ...
    def _show_simple_popup(self, title, message):
        layout = BoxLayout(orientation="vertical", padding=10, spacing=8)
        lbl_kwargs = {"text": message, "halign": "center", "valign": "middle", "color": (0.9, 0.85, 1, 1)}
        if FONT_PATH: lbl_kwargs["font_name"] = FONT_PATH
        lbl = Label(**lbl_kwargs)
        lbl.bind(size=lbl.setter("text_size"))

        btn = Button(text="OK", size_hint_y=None, height="40dp", background_normal="", background_color=(0.45, 0.20, 0.65, 1), color=(1, 1, 1, 1))
        if FONT_PATH: btn.font_name = FONT_PATH
                            ...
                            if requestCode == 1002 and intent:  # STT
                                try:
                                    ...
                                    if results and results.size() > 0:
                                        ...
                                        if self.app.main_screen.interpreter_active_side:
                                            self.app.main_screen.handle_interpreter_speech_result(recognized_text)
                                            self.app.main_screen.interpreter_active_side = None
                                        else:
                                            Clock.schedule_once(lambda dt: self._set_input(recognized_text))
                                except Exception as e:
                                    print(f"STT Process error: {e}")
                            elif requestCode == 1003:  # Camera
                                photo_path = getattr(self.app.main_screen, "photo_path", None)
                                if photo_path and os.path.exists(photo_path):
                                    Clock.schedule_once(lambda dt: self.app.main_screen.process_image_with_gemini(photo_path))
