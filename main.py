...
# ლოკალური Offline ლექსიკონი (ინტერნეტის გარეშე სარეზერვო თარგმანისთვის)
OFFLINE_DICT = {
    "hello": "გამარჯობა",
    "thanks": "გმადლობთ",
    "thank you": "გმადლობთ",
...
def is_internet_available():
    """ამოწმებს არის თუ არა მოწყობილობა ინტერნეტთან დაკავშირებული"""
    try:
...
class LingoLensUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.add_widget(Label(
            text='LingoLens Hybrid (Online/Offline) AI',
            font_size='18sp',
            bold=True,
            size_hint_y=0.08
        ))
        ...
        # 6. შედეგი
        label_kwargs = {
            'text': 'Ready. Online/Offline Engine Active.',
            'font_size': '15sp',
            'size_hint_y': 0.36
        }
        if GEORGIAN_FONT:
    ...
    # VOICE RECOGNITION WITH OFFLINE SUPPORT
    ...
    def start_speech(self, lang_code):
        try:
            ...
            # Offline ხმოვანი პაკეტების მხარდაჭერა
            intent.putExtra(RecognizerIntent.EXTRA_PREFER_OFFLINE, True)
            
            self.speech_recognizer.startListening(intent)
            self.result_label.text = f"Listening ({lang_code})..."
        except Exception as e:
    ...
    # HYBRID TRANSLATION ENGINE (ONLINE + OFFLINE FALLBACK)
    ...
    def _fetch_translation(self, text, src_name, tgt_name, tgt_code, auto_speak):
        ...
        if online:
            try:
                ...
                print(f"Online error, falling back to offline mode: {e}")

        # OFFLINE FALLBACK MODE
        clean_key = text.lower().strip()
        translated = OFFLINE_DICT.get(clean_key, f"[Offline Mode]: {text}")
        status_msg = f"{translated}\n\n(⚡ Note: Offline Mode active - No Internet)"
        Clock.schedule_once(lambda dt: self._update_result(status_msg, tgt_code, auto_speak))
    ...
    def _run_ai_ocr(self):
        if is_internet_available() and os.path.exists("ocr_frame.png") and GEMINI_API_KEY != "YOUR_GEMINI_API_KEY":
            ...
            Clock.schedule_once(lambda dt: self._set_ocr_text("Captured frame (Offline OCR requires internet or MLKit)"))
