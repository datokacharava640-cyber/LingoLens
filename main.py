...
# 1. შრიფტის რეგისტრაცია (ქართული ტექსტის სწორად გამოჩენისთვის)
GEORGIAN_FONT = None
        ...
        class ContinuousSpeechListener(PythonJavaClass):
            ...
            def onEndOfSpeech(self):
                if self.restart_callback:
                    Clock.schedule_once(lambda dt: self.restart_callback(), 0.1)

            @java_method('(I)V')
            def onError(self, error):
                if self.restart_callback:
                    Clock.schedule_once(lambda dt: self.restart_callback(), 0.3)
    ...
    def start_speech_recognizer(self, lang_code):
        ...
        try:
            ...
            listener = ContinuousSpeechListener(
                ...
                lambda: self.restart_speech_loop(lang_code)
            )
