...
class LingoLensUI(BoxLayout):
    def __init__(self, user_data_dir=".", **kwargs):
        ...
        # Output Box Layout
        output_box = BoxLayout(size_hint_y=0.3, spacing=5)
        self.output_label = Label(text="თარგმანი...", font_size='16sp', font_name=DEFAULT_FONT, size_hint_x=0.7)
        self.output_label.bind(size=self.output_label.setter('text_size'))
        
        btn_speak = Button(text="[AUDIO]", size_hint_x=0.3, font_name=DEFAULT_FONT, font_size='14sp')
    ...
    def start_speech_recognition(self, lang_code, is_georgian_speaker):
        if platform == 'android':
            try:
                ...
                # Android-ზე სისტემური Speech UI-ს გამოძახება
