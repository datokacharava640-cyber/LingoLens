...
class MainScreen(Screen):
    ...
    def speak_output_text(self, custom_text=None):
        ...
        threading.Thread(target=self._download_and_play_tts, args=(text, lang_code), daemon=True).start()

    def _download_and_play_tts(self, text, lang_code):
        try:
            app = App.get_running_app()
            base_dir = app.user_data_dir if (platform == 'android' and app) else "."
            audio_dir = os.path.join(base_dir, "tts_cache")
            os.makedirs(audio_dir, exist_ok=True)

            file_path = os.path.join(audio_dir, "speech.mp3")
            ...
            if res.status_code == 200:
                ...
                Clock.schedule_once(lambda dt: self._play_file_native(file_path), 0)
        except Exception as e:
            print(f"TTS Download Error: {e}")

    def _play_file_native(self, file_path):
        if platform == 'android':
            try:
                from jnius import autoclass
