...
APP_VERSION = "3.6.0"
VERCEL_SERVER_URL = "https://lingo-lens-eight.vercel.app/api/index"
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.source_lang = "ka"
        self.target_lang = "en"
        self.android_tts_engine = None
        self._init_native_tts()

    def _init_native_tts(self):
        """Android Native TextToSpeech-ის ინიციალიზაცია"""
        if platform == 'android':
            def _init_worker():
                try:
                    ...
                    # Java Listener-ის იმპლემენტაცია
                    ...
                    listener = TTSListener(self)
                    self.android_tts_engine = TextToSpeech(activity, listener)
                except Exception as e:
                    print(f"Android TTS Init Exception: {e}")

            Clock.schedule_once(lambda dt: _init_worker(), 0.5)
    ...
    def speak_output_text(self, custom_text=None):
        ...
        if platform == 'android' and self.android_tts_engine:
            try:
                from jnius import autoclass
                ...
                self.android_tts_engine.setLanguage(locale_obj)
                self.android_tts_engine.speak(text, TextToSpeech.QUEUE_FLUSH, None, None)
                return
            except Exception as e:
                print(f"Native Android Speech Error: {e}")

        # 2. Network Fallback (Android MediaPlayer / PC SoundLoader)
        threading.Thread(target=self._play_online_audio, args=(text, lang_code), daemon=True).start()

    def _play_online_audio(self, text, lang_code):
        try:
            ...
            if platform == 'android':
                ...
                filename = os.path.join(cache_dir, f"speech_{safe_hash}.mp3")
            ...
            print(f"Audio Fallback Exception: {e}")
    ...
    def open_dialog_mode(self):
        ...
        status_lbl = Label(text="ორმხრივი ხმოვანი დიალოგი\nდააჭირეთ ენის ღილაკს საუბრის დასაწყებად:", font_name=FONT_PATH, halign='center')
