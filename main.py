...
# Android Native Speech/TTS ინტეგრაცია jnius-ით
...
class MainScreen(Screen):
    ...
    def start_speech_to_text(self, lang_code="ka", on_complete_callback=None):
        """ხმოვანი შეყვანა Android Native Intent-ით"""
        if platform == 'android' and AndroidPythonActivity:
            try:
                from jnius import autoclass
                Intent = autoclass('android.content.Intent')
                RecognizerIntent = autoclass('android.speech.RecognizerIntent')
    ...
    def speak_output_text(self, custom_text=None):
        ...
        elif platform == 'android' and AndroidPythonActivity:
            try:
                from jnius import autoclass
    ...
    def open_dialog_mode(self):
        """სრულად ფუნქციონალური ორმხრივი ხმოვანი დიალოგი"""
        ...
        def _mic2_action(instance):
            ...
            # ენების დროებითი გაცვლა დიალოგისთვის
            self.swap_languages()
            self.start_speech_to_text(self.source_lang, _on_dialog_speech_captured)
