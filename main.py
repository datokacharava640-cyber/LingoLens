    ...
    def speak_audio(self, text, lang_code):
        try:
            ...
            class TTSInitListener(PythonJavaClass):
                ...
                def __init__(self, text, lang_code, tts_ref):
                    super().__init__()
                    self.text = text
                ...
                def onInit(self, status):
                    if status == TextToSpeech.SUCCESS and self.tts_ref:
                        self.tts_ref.setLanguage(Locale(self.lang_code))
                        self.tts_ref.speak(self.text, TextToSpeech.QUEUE_FLUSH, None, None)

            listener = TTSInitListener(text, lang_code, None)
            self.tts = TextToSpeech(PythonActivity.mActivity, listener)
            listener.tts_ref = self.tts
