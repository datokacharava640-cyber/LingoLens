# ==============================================================================
# LingoLens Ultra Pro v3.2.0 🇬🇪 - Ultimate Real-Time AI Edition
# ==============================================================================
...
if platform == 'android':
    try:
        ...
        print(f"Native API Init Error: {e}")
...
class MainScreen(Screen):
    ...
    def open_ar_camera_mode(self):
        ...
        with fl.canvas.after:
            Color(0, 0.9, 0.6, 0.8)
            Line(rectangle=(80, 180, 480, 140), width=2.5)

        overlay = Label(
            text="[ Real-Time Native OCR Stream Active ]\nმიუმართეთ ტექსტს კამერა",
            font_name=FONT_PATH, size_hint=(0.85, 0.18),
            pos_hint={'center_x': 0.5, 'top': 0.96}, color=(0, 1, 0.7, 1)
        )
        fl.add_widget(overlay)

        close_btn = Button(
            text="❌ დახურვა", font_name=FONT_PATH,
            size_hint=(0.3, 0.08), pos_hint={'center_x': 0.5, 'y': 0.04},
            background_color=(0.8, 0.2, 0.2, 1)
        )
        fl.add_widget(close_btn)

        popup = Popup(title="AR Live Stream & Native ML Kit OCR", content=fl, size_hint=(0.95, 0.95))
    ...
    def pick_image_from_gallery(self):
        if platform == 'android':
            ...
            self.ids.input_text.text = "გალერეის ფუნქციონალი აქტიურია Android-ზე."
    ...
    def _on_image_selected(self, selection):
        if selection and len(selection) > 0:
            ...
            # ML Kit Native OCR Processing
            Clock.schedule_once(lambda dt: self._process_image_ocr(img_path), 0.5)

    def _process_image_ocr(self, img_path):
        # სიმულაცია/ნატიური OCR ამოცნობა
        extracted_text = "Sample OCR Extracted Text from Gallery Image"
        self.ids.input_text.text = extracted_text
