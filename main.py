...
class AndroidKeyStoreBridge:
    ...
    def get_secure_key(alias="AZURE_API_KEY"):
        ...
        # Insert raw Azure API key or secure retrieval mechanism here
...
class LingoLensPro(MDApp):
    ...
    def show_popup(self, title, text, is_confirm=False, text_to_trans=""):
        ...
        if is_confirm:
            btns = [
                MDRaisedButton(text="დიახ", font_name="Roboto", on_release=confirm),
                MDRectangleFlatButton(text="არა", font_name="Roboto", on_release=lambda x: self.dialog.dismiss())
            ]
        else:
            btns = [MDRaisedButton(text="OK", font_name="Roboto", on_release=lambda x: self.dialog.dismiss())]
