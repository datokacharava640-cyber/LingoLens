...
class LingoLensUI(BoxLayout):
    def __init__(self, user_data_dir=".", **kwargs):
        ...
        # 3. Input Text Box
        self.input_text = TextInput(hint_text="შეიყვანეთ ტექსტი...", size_hint_y=0.25, font_name=DEFAULT_FONT)
        self.input_text.bind(text=self.on_text_change)
        self.add_widget(self.input_text)

        # 4. Output Text Box + Audio Button (კომპაქტური დიზაინი)
        output_box = BoxLayout(size_hint_y=0.2, spacing=5)
        ...
        # 5. Switch (გადამრთველი) დიალოგის რეჟიმისთვის
        toggle_bar = BoxLayout(size_hint_y=0.08, spacing=10)
        toggle_label = Label(text="ხმოვანი დიალოგის რეჟიმი:", font_name=DEFAULT_FONT, size_hint_x=0.7, halign='left')
        toggle_label.bind(size=toggle_label.setter('text_size'))
        
        self.mode_switch = Switch(active=True, size_hint_x=0.3)
        self.mode_switch.bind(active=self.toggle_voice_mode)
        
        toggle_bar.add_widget(toggle_label)
        toggle_bar.add_widget(self.mode_switch)
        self.add_widget(toggle_bar)

        # 6. Live Dialog Voice Buttons (ხმოვანი ღილაკები)
        self.voice_box = BoxLayout(size_hint_y=0.18, spacing=8)
        
        self.btn_foreigner = Button(
            text="🎤 უცხოელის საუბარი", 
            font_name=DEFAULT_FONT,
            halign='center'
        )
        self.btn_foreigner.bind(on_press=self.listen_foreigner)

        self.btn_georgian = Button(
            text="🎤 ჩემი საუბარი (ქართული)", 
            font_name=DEFAULT_FONT,
            halign='center'
        )
        self.btn_georgian.bind(on_press=self.listen_georgian)

        self.voice_box.add_widget(self.btn_foreigner)
        self.voice_box.add_widget(self.btn_georgian)
        self.add_widget(self.voice_box)

    def toggle_voice_mode(self, instance, value):
        # გადამრთველით ხმოვანი ღილაკების გამოჩენა/დამალვა
        if value:
            self.voice_box.disabled = False
            self.voice_box.opacity = 1.0
        else:
            self.voice_box.disabled = True
            self.voice_box.opacity = 0.3
    ...
    # --- VOICE DIALOG / STT RECOGNITION ---
    def listen_foreigner(self, instance):
        lang_code = SRC_LANGUAGES.get(self.src_spinner.text, "en")
        if lang_code == "auto":
            lang_code = "en"
        self.start_speech_recognition(lang_code, is_georgian_speaker=False)

    def listen_georgian(self, instance):
        self.start_speech_recognition("ka-GE", is_georgian_speaker=True)

    def start_speech_recognition(self, lang_code, is_georgian_speaker):
        if platform == 'android':
            try:
