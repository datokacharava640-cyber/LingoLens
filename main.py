...
class MainScreen(Screen):
    ...
    def _download_and_play_tts_stream(self, text, lang_code):
        """ქართული ენისთვის edge-tts ნეირონული ხმა, ხოლო სხვა ენებისთვის Google TTS"""
        try:
            if platform == 'android':
                app = App.get_running_app()
            ...
            filename = os.path.abspath(os.path.join(cache_dir, f"speech_{safe_hash}.mp3"))

            # 1. ქართული ენის ნეირონული გაჟღერება edge-tts-ით
            if lang_code.startswith("ka"):
                import edge_tts
            ...
            # 2. სხვა ენებისთვის სტანდარტული Google TTS
            else:
            ...
            # 3. აუდიო ფაილის დაკვრა Android / PC-ზე
