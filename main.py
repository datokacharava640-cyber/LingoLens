...
class MainScreen(Screen):
    ...
    def speak_output_text(self, custom_text=None):
        ...
        threading.Thread(target=self._play_universal_audio, args=(text, lang_code), daemon=True).start()

    def _play_universal_audio(self, text, lang_code):
        try:
            if platform == 'android':
                app = App.get_running_app()
                base_dir = app.user_data_dir if app else "."
            ...
            # ფაილის სახელი ენისა და ტექსტის ჰეშის მიხედვით
            ...
            if os.path.exists(filename):
                # წინა აუდიოს გაჩერება თუ უკვე უკრავდა
            ...
            print(f"Universal Audio Playback Error: {e}")
