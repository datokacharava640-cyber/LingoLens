...
class MainScreen(Screen):
    ...
    def open_camera_ocr(self):
        ...
        if platform == 'android':
            try:
                ...
                log_error(e)
                self._show_simple_popup("Camera", f"Error: {str(e)}")
        else:
            self._show_simple_popup("Camera", "Available on Android only")
    ...
    def open_language_menu(self, mode):
        ...
        popup = Popup(title="Select Language", content=main_layout, size_hint=(0.9, 0.8))
    ...
    def perform_translation(self, text):
        ...
        url = f"https://translate.googleapis.com/translate_a/single?client=gtx&sl={self.source_lang}&tl={self.target_lang}&dt=t&q={requests.utils.quote(text)}"
