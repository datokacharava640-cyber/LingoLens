...
class CameraARWidget(BoxLayout):
    ...
    def update_frame(self, dt):
        ...
        # Throttling: Send API request max once per second (1.0 sec interval) to fix Lag/FPS
        current_time = time.time()
        if not self.is_processing and (current_time - self.last_api_call > 1.0):
...
class MainScreen(Screen):
    ...
    def smart_offline_translate(self, text):
        ...
        # SQLite ბაზიდან ოფლაინ თარგმნის მცდელობა
        if os.path.exists("offline_dict.db"):
            try:
                conn = sqlite3.connect("offline_dict.db")
                cursor = conn.cursor()
                for w in words:
                    cursor.execute("SELECT target_word FROM dictionary WHERE source_word=? AND src_lang=? AND tgt_lang=?", 
                                   (w, self.source_lang, self.target_lang))
                    res = cursor.fetchone()
                    translated_words.append(res[0] if res else w)
                conn.close()
                return " ".join(translated_words)
            except Exception:
                pass

        # Fallback ლოკალურ მცირე ლექსიკონზე
    ...
    def _play_georgian_tts(self, text):
        try:
            ...
            # თუ აუდიო ფაილი ქეშშია, დაუკარი ონლაინ რეჟიმის გარეშე
            if os.path.exists(filename):
                sound = SoundLoader.load(filename)
                if sound: sound.play()
                return

            # თუ არ არის ქეშში, ჩამოტვირთე Google TTS-ით და შეინახე
            ...
            print(f"Georgian TTS Audio Error: {e}")
