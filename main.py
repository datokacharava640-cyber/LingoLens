...
# Spinner-ის ელემენტებისთვის ფონტის გადაცემა
class CustomSpinnerOption(SpinnerOption):
    font_name = "Roboto" if os.path.exists(FONT_PATH) else "Roboto"
...
class LingoLensApp(App):
    ...
    def enable_camera(self):
        try:
            if hasattr(self, 'camera') and isinstance(self.camera, Camera):
    ...
    def build(self):
        ...
        font_to_use = "Roboto" if os.path.exists(FONT_PATH) else "Roboto"
        ...
        try:
            ...
            self.camera = Label(text="Camera Unavailable", size_hint_y=0.28, font_name=font_to_use)
