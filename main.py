...
# Pure Python PDF Parser (Buildozer-ის შეცდომების თავიდან ასაცილებლად)
try:
    import pypdf
except ImportError:
    pypdf = None
...
class DatabaseManager:
    ...
    def create_tables(self):
        with self.get_connection() as conn:
            ...
            """)
            # 3. EdTech Flashcards (Anki-ს ინტერვალური განმეორება)
            cursor.execute("""
...
class ServiceManager:
    ...
    def toggle_voice_stream(self, callback_update):
        """ რეალური დროის Full-Duplex ხმოვანი თარგმანის სიმულაცია / ინტეგრაცია """
        self.is_listening = not self.is_listening
        if self.is_listening:
            callback_update("🎙️ უწყვეტი ხმოვანი რეჟიმი აქტიურია (გასაჩერებლად დააჭირეთ კვლავ)...")
        else:
    ...
    def start_overlay(self):
        if platform == 'android':
            try:
                ...
                return "🫧 Floating Overlay აქტიურია! (ეკრანის თარგმნა)"
            except Exception as e:
...
class MainLayout(BoxLayout):
    ...
    def on_file_selected(self, selection):
        if selection and len(selection) > 0:
            ...
            if doc_text:
                self.text_input.text = doc_text[:600]
