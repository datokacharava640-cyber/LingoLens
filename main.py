    ...
    def translate_offline_fuzzy(self, text):
        try:
            ...
            with self.get_connection() as conn:
                ...
                for w in words:
                    if w in dict_data:
                        res.append(dict_data[w])
                    else:
                        matches = difflib.get_close_matches(w, dict_data.keys(), n=1, cutoff=0.7)
                        if matches:
...
# ---------------------------------------------------------
# 2. HYBRID SERVICE MANAGER (Proxy / Direct Gemini API)
# ---------------------------------------------------------
class ServiceManager:
    ...
    def translate_text(self, text):
        ...
        # Option A: Use Backend Proxy (Most Secure)
        if proxy_url:
            try:
                res = self.session.post(f"{proxy_url.rstrip('/')}/translate", json={"text": text}, timeout=8)
        ...
        if api_key:
            ...
            try:
                res = self.session.post(f"{self.direct_api_url}?key={api_key}", json=payload, timeout=8)
                if res.status_code == 200:
    ...
    def translate_image(self, image_path):
        ...
        try:
            ...
            # Direct API Route
            if api_key:
                prompt = "Extract text from image and translate to Georgian/English. Return ONLY JSON with key 'translation'."
                payload = {
                    "contents": [{
                        "parts": [
                            {"text": prompt},
...
class LingoLensUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        title = Label(text="LingoLens Ultra Pro", font_size='18sp', bold=True, halign='left')
        ...
        # Action Buttons
        actions = BoxLayout(size_hint_y=0.12, spacing=5)
    ...
    def open_settings(self, instance):
        ...
        content.add_widget(Label(text="Backend Proxy URL (უსაფრთხო):"))
        content.add_widget(proxy_input)
        content.add_widget(Label(text="Direct Gemini API Key (სარეზერვო):"))
