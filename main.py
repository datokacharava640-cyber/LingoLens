...
def request_android_permissions(dt=None):
    if platform == 'android':
        try:
...
class LingoLensUI(BoxLayout):
    ...
    def trigger_realtime_translate(self):
        ...
        url = f"https://translate.googleapis.com/translate_a/single?client=gtx&sl={src_code}&tl={target_code}&dt=t&q={text}"

        # უსაფრთხო Async Network Request კრაშის გარეშე
    ...
    def on_translation_error(self, req, error):
        self.output_label.text = "[ქსელის შეცდომა ან ინტერნეტი გამორთულია]"
...
class LingoLensApp(App):
    def build(self):
        ui = LingoLensUI()
        Clock.schedule_once(request_android_permissions, 1)
        return ui
