...
# ქართული ფონტის გლობალური რეგისტრაცია (კუბიკების თავიდან ასაცილებლად)
...
class RealTimeAudioVAD:
    def __init__(self, on_speech_complete_callback, silence_threshold=400, pause_duration=1.2):
...
class MainLayout(BoxLayout):
    ...
    def toggle_listening(self, instance):
        if not self.is_active:
            ...
            self.status_label.text = "გასმენთ... (Real-Time VAD)"
    ...
    def start_foreground_service(self):
        if platform == 'android':
            try:
                ...
                print(f"Service launch error: {e}")
