...
# ენების სია (ლათინური და ქართული დასახელებებით, ოთხკუთხედების თავიდან ასაცილებლად)
SRC_LANGUAGES = {
    "ავტო ამოცნობა": "auto",
    "ქართული": "ka",
    "English": "en",
...
class LingoLensUI(BoxLayout):
    ...
    def on_text(self, instance, value):
        ...
        src = SRC_LANGUAGES.get(self.src_spinner.text, "auto")
        target = TARGET_LANGUAGES.get(self.target_spinner.text, "en")
        url = f"https://translate.googleapis.com/translate_a/single?client=gtx&sl={src}&tl={target}&dt=t&q={quote(text)}"

        # გადავცემთ გაგზავნილ ტექსტს (text)req-ის კონტექსტში
        UrlRequest(
            url, 
            on_success=lambda req, res: self.on_success(req, res, text), 
            on_error=self.on_err, 
            on_failure=self.on_err, 
            timeout=5
        )
