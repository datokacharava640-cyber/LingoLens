"""
LingoLens Ultra Pro v3.6.0
==================================================
პროექტის ავტორი: დავით კაჭარავა (Davit Kacharava)
შექმნის ადგილი: საქართველო (Georgia)
თარიღი: 5 სექტემბერი, 2026 წელი

ლიცენზიები და შეთანხმებები:
- LingoLens Software License: MIT License (c) 2026 Davit Kacharava
- Google Gemini API: Subject to Google AI Terms of Service & Gemini API License
- OpenAI API (AI Engines): Subject to OpenAI Terms of Use and Commercial Licenses
- Picovoice Porcupine & PvRecorder: Apache License 2.0 / Picovoice Terms of Use
- Kivy Framework: MIT License
==================================================
"""
...
# 1. უსაფრთხო იმპორტი Crash-ის ასარიდებლად
...
LANGUAGES = {
    ...
    "Lao (ລາວ)": "lo",
    "Burmese (မြန်မာ)": "my",
    "Georgian (Old Script - ასომთავრული)": "ka",
    "Latin (Latina)": "la",
    "Esperanto": "eo"
}
...
[b]პროექტის შექმნელი:[/b] {PROJECT_AUTHOR}
[b]ქვეყანა:[/b] {PROJECT_LOCATION}
[b]თარიღი:[/b] {PROJECT_DATE}

----------------------------------------
[b]ლიცენზიები და ინტელექტუალური საკუთრება:[/b]

1. [b]LingoLens Software License[/b]
   Copyright (c) 2026 {PROJECT_AUTHOR}. Released under the MIT License.

2. [b]Google Gemini API[/b]
   Integrated under Google AI Terms of Service and Gemini API Terms of Use.

3. [b]OpenAI / AI Translation Services[/b]
   Powered by OpenAI API endpoints, licensed for commercial and open integration.

4. [b]Picovoice Porcupine & PvRecorder[/b]
   Voice Wake-Word Engine licensed under Apache License 2.0 / Picovoice Commercial License.

5. [b]Kivy Engine[/b]
   Open-source Python UI framework under the MIT License.
"""
        lbl = Label(
            text=info_text,
            markup=True,
        ...
        popup = Popup(
            title="პროექტის შესახებ & ლიცენზიები",
    ...
    def start_wakeword_listener(self):
        if not PORCUPINE_AVAILABLE or PICOVOICE_ACCESS_KEY == "YOUR_PICOVOICE_ACCESS_KEY_HERE":
            print("Wake Word Engine disabled: Key not provided or library missing.")
            return
    ...
    # მსოფლიოსა და ქართული ენის სრული გრამატიკული ანალიზატორი
    def translate_with_grammar(self):
        text = self.ids.input_text.text.strip()
        if not text:
            return
        self.ids.output_text.text = "[AI გრამატიკული და ენობრივი ანალიზი მიმდინარეობს...]"
        url = f"{VERCEL_BASE_URL}/api/index"
        
        prompt_with_grammar_instructions = (
            f"Analyze the following text grammatically in detail (covering syntax, verb tenses, word roots, and structure) "
            f"specifically for {self.source_lang} and {self.target_lang}. Provide explanations in Georgian:\n\n{text}"
        )
        
        payload = {
            "text": prompt_with_grammar_instructions,
            "source_lang": self.source_lang[:2],
            "target_lang": self.target_lang[:2],
            "mode": "grammar"
        }
        ...
        def _on_result(success, response):
            if success and isinstance(response, dict):
                ...
                if not grammar:
                    grammar = translated
                res_display = f"თარგმანი:\n{translated}\n\n--- AI გრამატიკული & ენობრივი ანალიზი ---\n{grammar}"
    ...
    # 2. უსაფრთხო STT Android UI Thread-ზე
    ...
    # 3. უსაფრთხო Android Audio Player (file:// პრეფიქსის გარეშე)
            ...
            if res.status_code == 200:
                ...
                if os.path.exists(filepath) and os.path.getsize(filepath) > 0:
                    if platform == 'android':
                        ...
                        player = MediaPlayer()
                        player.setDataSource(filepath) # სწორი გზა URI შეცდომის გარეშე
                        player.prepare()
                        player.start()
                    else:
