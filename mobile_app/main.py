...
VERCEL_BASE_URL = "[https://lingo-lens-eight.vercel.app](https://lingo-lens-eight.vercel.app)"
API_AUTH_TOKEN = "Bearer LINGOLENS_SECURE_TOKEN_2026"
FONT_PATH = "font.ttf" if os.path.exists("font.ttf") else "Roboto"

# ----------------------------------------------------
# 1. ენების სრული ბაზა (100+ ენა) - [შესწორებული სინტაქსი]
# ----------------------------------------------------
LANGUAGES = {
...
class GeorgianFlagBackground(Widget):
    ...
    def start_bg_animation(self):
        if not self.is_animating:
            ...
            Clock.schedule_interval(self.animate, 1 / 15.0) # 15 FPS ოპტიმიზაციისთვის
...
class MainScreen(Screen):
    ...
    def swap_languages(self):
        ...
        s_name = next((k for k, v in LANGUAGES.items() if v == self.source_lang), self.source_lang)
        t_name = next((k for k, v in LANGUAGES.items() if v == self.target_lang), self.target_lang)
        
        self.ids.btn_source_lang.text = s_name
        self.ids.btn_target_lang.text = t_name
    ...
    def trigger_translation(self, text):
        ...
        def handle_response(success, data):
            if success and isinstance(data, dict) and "translatedText" in data:
    ...
    def analyze_and_think(self):
        ...
        if not text:
            self.ids.output_text.text = "გთხოვთ, შეიყვანოთ ტექსტი ანალიზისთვის."
            return
    ...
    def start_speech_to_text(self, lang_code):
        ...
        def on_text_callback(msg):
            self.ids.audio_viz.stop_animation()
            self.ids.input_text.text = msg

        NativeSpeechManager.start_listening(lang_code, on_text_callback)
    ...
    def open_interpreter_mode(self):
        ...
        lbl = Label(text="🗣️ დიალოგის რეჟიმი\nორენოვანი ცოცხალი თარგმანი", font_name=FONT_PATH, halign="center")
        box.add_widget(lbl)
        popup = Popup(title="Interpreter Mode", content=box, size_hint=(0.85, 0.5))
        popup.open()
    ...
    def open_dictionary_hub(self):
        ...
        for k, v in OFFLINE_DICTIONARY.items():
            btn = Button(text=f"{k.upper()} - {v['ka']}", font_name=FONT_PATH, size_hint_y=None, height='40dp')
            list_box.add_widget(btn)

        scroll.add_widget(list_box)
        box.add_widget(scroll)
        popup = Popup(title="📖 ოფლაინ ლექსიკონი", content=box, size_hint=(0.9, 0.8))
        popup.open()

    def open_quiz_mode(self):
        trigger_vibration()
        quiz_item = random.choice(DAILY_QUIZ)
        box = BoxLayout(orientation='vertical', padding=10, spacing=8)
        lbl = Label(text=quiz_item["q"], font_name=FONT_PATH, font_size='16sp')
        box.add_widget(lbl)

        popup = Popup(title="📝 დღიური ქვიზი", content=box, size_hint=(0.85, 0.6))

        def check_ans(opt):
            if opt == quiz_item["a"]:
                lbl.text = "✅ სწორია!"
            else:
                lbl.text = f"❌ არასწორია! სწორია: {quiz_item['a']}"
            Clock.schedule_once(lambda dt: popup.dismiss(), 1.5)

        for opt in quiz_item["options"]:
            btn = Button(text=opt, font_name=FONT_PATH, size_hint_y=None, height='40dp')
            btn.bind(on_release=lambda instance, o=opt: check_ans(o))
            box.add_widget(btn)

        popup.open()

# ----------------------------------------------------
# 9. Application Entry Point & Safe Permissions Manager
# ----------------------------------------------------
