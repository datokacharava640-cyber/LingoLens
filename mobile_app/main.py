...
# Android-ის სპეციფიკური ბინდინგი
if platform == 'android':
    try:
...
# ----------------------------------------------------
# 5. Native Speech Manager (გამართული Android STT და TTS)
# ----------------------------------------------------
class NativeSpeechManager:
    ...
    def start_listening(lang_code, callback_text=None):
        if platform == 'android':
            try:
                ...
                # Auto რეჟიმში ნაგულისხმებად ქართულს იყენებს
                req_lang = "ka-GE" if lang_code == "ka" or lang_code == "auto" else lang_code
                intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE, req_lang)
                currentActivity.startActivityForResult(intent, 1001)
            except Exception as e:
                log_error(e)
                if callback_text: callback_text("ხმის ამოცნობის ფუნქცია ჩართულია...")
        else:
            if callback_text: callback_text("[Desktop Simulation]: ხმოვანი შეყვანა აქტიურია...")
    ...
    def speak_text(text, lang_code, speed=1.0):
        ...
        if platform == 'android':
            try:
                ...
                def _init_and_speak():
                    if NativeSpeechManager._tts_instance is None:
                        ...
                        time.sleep(0.5)  # ინიციალიზაციის მცირე დაყოვნება
            ...
            print(f"[Desktop Simulation TTS ({lang_code} / {speed}x)]: {text}")
...
class MainScreen(Screen):
    ...
    def open_language_menu(self, mode):
        ...
        def populate_languages(query=""):
            ...
            for name, code in LANGUAGES.items():
                ...
                if clean_q and (clean_q not in name.lower() and clean_q not in code.lower()): continue
                btn = Button(text=name, font_name=FONT_PATH, size_hint_y=None, height='40dp', background_color=(0.18, 0.22, 0.3, 1))
    ...
    def swap_languages(self):
        ...
        src_name = next((k for k, v in LANGUAGES.items() if v == self.source_lang), self.source_lang)
        tgt_name = next((k for k, v in LANGUAGES.items() if v == self.target_lang), self.target_lang)
        
        self.ids.btn_source_lang.text = src_name
        self.ids.btn_target_lang.text = tgt_name

        out_txt = self.ids.output_text.text
        if out_txt and not out_txt.startswith("["):
            self.ids.input_text.text = out_txt
            self.trigger_translation(out_txt)
    ...
    def trigger_translation(self, text):
        ...
        def on_result(success, response):
            self.ids.audio_viz.stop_animation()
            if success and isinstance(response, dict) and "translation" in response:
                res = response["translation"]
                self.ids.output_text.text = res
                db.add_history(self.source_lang, self.target_lang, text, res)
            else:
                cached = db.search_offline_cache(self.source_lang, self.target_lang, text)
                if cached:
                    self.ids.output_text.text = cached
                else:
                    self.ids.output_text.text = f"[Offline Translation Error]: {response}"

        AsyncTranslateEngine.async_post_request(f"{VERCEL_BASE_URL}/api/translate", payload, on_result)

    def analyze_and_think(self):
        trigger_vibration()
        txt = self.ids.input_text.text.strip()
        if not txt:
            self.ids.output_text.text = "გთხოვთ შეიყვანოთ ტექსტი AI ანალიზისთვის."
            return
    ...
    def speak_output_text(self):
        trigger_vibration()
        txt = self.ids.output_text.text
        NativeSpeechManager.speak_text(txt, self.target_lang, self.tts_speed)

    def start_speech_to_text(self, lang):
        trigger_vibration()
        NativeSpeechManager.start_listening(lang)
    ...
    def open_interpreter_mode(self):
        ...
        log_input = TextInput(readonly=True, hint_text="ორმხრივი დიალოგის ისტორია...", font_name=FONT_PATH)
        
        btn_box = BoxLayout(size_hint_y=None, height='45dp', spacing=6)
        btn1 = Button(text="🎤 პირი A (KA)", font_name=FONT_PATH, background_color=(0.2, 0.6, 0.8, 1))
        btn2 = Button(text="🎤 პირი B (EN)", font_name=FONT_PATH, background_color=(0.8, 0.4, 0.2, 1))

        btn_box.add_widget(btn1)
        btn_box.add_widget(btn2)
        box.add_widget(log_input)
        box.add_widget(btn_box)

        popup = Popup(title='🗣️ რეალურ დროში თარჯიმანი', title_font=FONT_PATH, content=box, size_hint=(0.95, 0.85))
        
        def _listen_a(instance):
            NativeSpeechManager.start_listening('ka')
        def _listen_b(instance):
            NativeSpeechManager.start_listening('en')

        btn1.bind(on_release=_listen_a)
        btn2.bind(on_release=_listen_b)
        popup.open()

    def open_dictionary_hub(self):
        trigger_vibration()
        box = BoxLayout(orientation='vertical', padding=8, spacing=6)
        search = TextInput(hint_text="შეიყვანეთ ინგლისური სიტყვა...", font_name=FONT_PATH, size_hint_y=None, height='40dp')
        res_label = TextInput(readonly=True, font_name=FONT_PATH)

        def _do_search(instance, text):
            word = text.strip().lower()
            if word in OFFLINE_DICTIONARY:
                d = OFFLINE_DICTIONARY[word]
                res_label.text = f"სიტყვა: {word}\nთარგმანი: {d['ka']}\nმეტყველების ნაწილი: {d['pos']}\nსინონიმები: {', '.join(d['syn'])}\nანტონიმები: {', '.join(d['ant'])}\nიდიომა: {d['idiom']}"
            else:
                res_label.text = "სიტყვა ვერ მოიძებნა ლოკალურ ბაზაში."

        search.bind(text=_do_search)
        box.add_widget(search)
        box.add_widget(res_label)
        popup = Popup(title='📖 ინტელექტუალური ლექსიკონი', title_font=FONT_PATH, content=box, size_hint=(0.9, 0.8))
        popup.open()

    def open_quiz_mode(self):
        trigger_vibration()
        if not DAILY_QUIZ: return
        item = random.choice(DAILY_QUIZ)
        
        box = BoxLayout(orientation='vertical', padding=10, spacing=8)
        lbl = Label(text=item["q"], font_name=FONT_PATH, font_size='16sp', size_hint_y=None, height='50dp')
        box.add_widget(lbl)

        popup = Popup(title='📝 ენის ქვიზი', title_font=FONT_PATH, content=box, size_hint=(0.85, 0.6))
        ...
        def _check_ans(ans):
            ...
            if ans == item["a"]:
                ...
                lbl.text = f"❌ არასწორია! სწორია: {item['a']}"
            Clock.schedule_once(lambda dt: popup.dismiss(), 1.5)

        for opt in item["options"]:
            btn = Button(text=opt, font_name=FONT_PATH, size_hint_y=None, height='40dp', background_color=(0.2, 0.3, 0.4, 1))
            btn.bind(on_release=lambda instance, o=opt: _check_ans(o))
            box.add_widget(btn)
    ...
    def open_file_translator(self):
        ...
        if platform != 'desktop':
            ...
            self.ids.input_text.text = "[Desktop Simulation]: ფაილის შიგთავსი ჩვიტვირთე..."
        ...
        if selection and len(selection) > 0:
            try:
                with open(selection[0], 'r', encoding='utf-8') as f:
                    content = f.read()
                    self.ids.input_text.text = content[:1000]
            except Exception as e:
                log_error(e)

# ----------------------------------------------------
# 9. App Application Class (Android-ის Event Listener-ით)
# ----------------------------------------------------
class LingoLensApp(App):
    def build(self):
        self.title = f"LingoLens Ultra Pro v{APP_VERSION}"
        if platform == 'android' and android_bind:
            android_bind(on_activity_result=self.on_activity_result)
    ...
    def on_activity_result(self, request_code, result_code, intent):
        """ Android-ის მიკროფონის შედეგის მიღება """
        if request_code == 1001 and result_code == -1:  # RESULT_OK = -1
            try:
                ...
                if results and len(results) > 0:
                    ...
                    main_screen.ids.input_text.text = recognized_text
            except Exception as e:
                log_error(e)
