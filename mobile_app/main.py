"""
LingoLens Ultra Pro v8.0.0 (External Hardware Mic & Enterprise Edition)
==================================================
...
DAILY_WORDS = [
    {"word": "Resilience", "translation": "მდგრადობა / გაძლება", "example": "Resilience is key to success."},
    {"word": "Innovation", "translation": "ინოვაცია / სიახლე", "example": "Innovation drives growth."},
    {"word": "Perseverance", "translation": "შეუპოვრობა", "example": "Perseverance leads to victory."},
    {"word": "Adaptability", "translation": "ეგუებადობა", "example": "Adaptability helps in change."}
]
    ...
    BoxLayout:
        ...
        BoxLayout:
            ...
            Button:
                ...
                on_release: root.show_word_of_the_day()
...
class MainScreen(Screen):
    ...
    def show_word_of_the_day(self):
        trigger_vibration()
        item = random.choice(DAILY_WORDS)
        content = BoxLayout(orientation='vertical', padding=12, spacing=8)
        lbl_word = Label(text=f"[size=18sp][b]{item['word']}[/b][/size]\n\nთარგმანი: {item['translation']}\nმაგალითი: {item['example']}", markup=True, font_name=FONT_PATH, halign='center')
        content.add_widget(lbl_word)
        close_btn = Button(text="დახურვა", font_name=FONT_PATH, size_hint_y=None, height='40dp', background_color=(0.3, 0.3, 0.3, 1))
        content.add_widget(close_btn)
        popup = Popup(title="💡 დღის სიტყვა (Flashcard)", title_font=FONT_PATH, content=content, size_hint=(0.85, 0.45))
    ...
    def _delayed_translate(self, text):
        ...
        payload = {"text": text, "source_lang": self.source_lang[:2] if self.source_lang != "auto" else "auto", "target_lang": self.target_lang[:2], "mode": "standard"}
        ...
        def _on_result(success, response):
            if success and isinstance(response, dict):
                ...
                if cached:
                    self.ids.output_text.text = f"[Offline/ქეშირებული]\n{cached}"
                else:
    ...
    def start_speech_to_text(self, lang_code="ka"):
        ...
        if platform == 'android':
            try:
                ...
                # გარე / Bluetooth მიკროფონის პრიორიტეტული ჩართვა (SCO Mode)
                ...
                def _on_activity_result(req, res, intent):
                    ...
                    # ჩაწერის დასრულებისას SCO-ს გათიშვა
