"""
LingoLens Ultra Pro v3.7.0
==================================================
პროექტის ავტორი: დავით კაჭარავა (Davit Kacharava)
შექმნის ადგილი: საქართველო (Georgia)
თარიღი: 2026 წელი

ლიცენზიები:
- LingoLens License: MIT License (c) 2026 Davit Kacharava
- Google Gemini & OpenAI API Services
- Picovoice Porcupine & Kivy Framework
==================================================
"""
...
try:
    ...
    print(f"Porcupine missing or disabled: {e}")
...
class AnimatedBackground(Widget):
    ...
    def update_canvas(self, *args):
        ...
        with self.canvas.before:
            ...
            # მოძრავი ანიმაციური ელემენტები
            c1_x = self.x + (self.width * 0.3) + math.sin(self.angle) * 40
            c1_y = self.y + (self.height * 0.7) + math.cos(self.angle) * 30
            Color(0.1, 0.3, 0.6, 0.25)
            Ellipse(pos=(c1_x, c1_y), size=(180, 180))

            c2_x = self.x + (self.width * 0.6) + math.cos(self.angle * 0.8) * 50
            c2_y = self.y + (self.height * 0.2) + math.sin(self.angle * 0.8) * 40
            Color(0.5, 0.1, 0.6, 0.2)
            Ellipse(pos=(c2_x, c2_y), size=(220, 220))
...
<MainScreen>:
    ...
    BoxLayout:
        ...
        BoxLayout:
            ...
            Label:
                text: "LingoLens Ultra v{APP_VERSION}"
                bold: True
                font_size: '13sp'
                font_name: '{FONT_PATH}'
                color: 0.2, 0.7, 1, 1
            ...
            Button:
                ...
                color: 1, 1, 1, 1
                on_release: root.open_info_popup()
        ...
        BoxLayout:
            ...
            Button:
                ...
                font_name: '{FONT_PATH}'
                background_color: 0.12, 0.15, 0.22, 0.8
                color: 1, 1, 1, 1
        ...
        BoxLayout:
            ...
            TextInput:
                id: output_text
                hint_text: "თარგმანი და AI ანალიზი გამოჩნდება აქ..."
                font_name: '{FONT_PATH}'
                readonly: True
                background_color: 0, 0, 0, 0
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.is_listening_wakeword = False
    ...
    def open_info_popup(self):
        ...
        info_text = f"""[b]LingoLens Ultra Pro v{APP_VERSION}[/b]

[b]ავტორი:[/b] {PROJECT_AUTHOR}
[b]ლოკაცია:[/b] {PROJECT_LOCATION}

[b]ფუნქციები:[/b]
1. Live-Translate & AI Grammatical Engine
2. აზროვნება & განვითარება (Deep AI Context Analysis)
3. Native STT & Server TTS მეტყველების სისტემა
4. Live Dialogue & Voice Assistant Mode
5. SQLite ოფლაინ ისტორია
"""
        lbl = Label(text=info_text, markup=True, font_name=FONT_PATH, size_hint_y=None, font_size='13sp', color=(0.9, 0.9, 0.9, 1))
        lbl.bind(texture_size=lambda inst, val: setattr(inst, 'height', val[1]))
        scroll.add_widget(lbl)
        content.add_widget(scroll)

        close_btn = Button(text="დახურვა", font_name=FONT_PATH, size_hint_y=None, height='40dp', background_color=(0.2, 0.5, 0.8, 1))
        content.add_widget(close_btn)

        popup = Popup(title="პროექტის შესახებ", title_font=FONT_PATH, content=content, size_hint=(0.9, 0.7))
        close_btn.bind(on_release=popup.dismiss)
        popup.open()

    def start_wakeword_listener(self):
        if not PORCUPINE_AVAILABLE or PICOVOICE_ACCESS_KEY == "YOUR_PICOVOICE_ACCESS_KEY_HERE":
            return
        self.is_listening_wakeword = True
        threading.Thread(target=self._wakeword_worker, daemon=True).start()

    def _wakeword_worker(self):
        porcupine = None
        recorder = None
        try:
            porcupine = pvporcupine.create(access_key=PICOVOICE_ACCESS_KEY, keywords=["porcupine", "bumblebee"])
            recorder = PvRecorder(device_index=-1, frame_length=porcupine.frame_length)
            recorder.start()

            while self.is_listening_wakeword:
                pcm = recorder.read()
                result = porcupine.process(pcm)
                if result >= 0:
                    Clock.schedule_once(lambda dt: self.on_wakeword_detected(), 0)
        except Exception as e:
    ...
    def on_wakeword_detected(self):
        self.activate_voice_assistant(auto_listen=True)
    ...
    def analyze_and_think(self):
        """აზროვნება da განვითარება - სიღრმისეული AI კონტექსტური ანალიზი"""
        text = self.ids.input_text.text.strip()
        if not text:
            return
        self.ids.output_text.text = "[AI აზროვნებს და გაანალიზებს კონტექსტს...]"
        
        prompt = (
            f"Provide a deep reasoning and contextual analysis for the following text in Georgian. "
            f"Explain meaning, intent, vocabulary development, and practical usage tips:\n\n'{text}'"
        )
        url = f"{VERCEL_BASE_URL}/api/index"
        payload = {
            "text": prompt,
        ...
        def _on_result(success, response):
            if success and isinstance(response, dict):
                ...
                self.ids.output_text.text = f"--- AI აზროვნება & განვითარება ---\n\n{res}"
            else:
                self.ids.output_text.text = "[ანალიზის შეცდომა]"
    ...
    def open_dialog_mode(self):
        ...
        status_lbl = Label(
            text="ორმხრივი ხმოვანი დიალოგი\nდააჭირეთ ღილაკს საუბრის დასაწყებად:",
            font_name=FONT_PATH,
            halign='center',
            color=(0.2, 0.8, 1, 1)
        )
        ...
        popup = Popup(title="Live Dialogue Mode", title_font=FONT_PATH, content=dialog_layout, size_hint=(0.85, 0.6))
        ...
        btn_mic1.bind(on_release=lambda x: self.start_speech_to_text(self.source_lang, _on_dialog_speech_captured))
        btn_mic2.bind(on_release=lambda x: (self.swap_languages(), self.start_speech_to_text(self.source_lang, _on_dialog_speech_captured)))
        close_btn.bind(on_release=popup.dismiss)
    ...
    def activate_voice_assistant(self, auto_listen=False):
        layout = BoxLayout(orientation='vertical', padding=15, spacing=10)
        lbl_status = Label(text="LingoLens AI ასისტენტი\n\nდააჭირეთ ღილაკს და დასვით კითხვა:", font_name=FONT_PATH, halign='center', color=(0.2, 0.8, 1, 1))
        layout.add_widget(lbl_status)

        btn_listen = Button(text="ლაპარაკი", font_name=FONT_PATH, size_hint_y=None, height='50dp', background_color=(0.5, 0.2, 0.8, 1))
        ...
        self.assistant_popup = Popup(title="LingoLens Voice Assistant", title_font=FONT_PATH, content=layout, size_hint=(0.9, 0.6))

        def _listen_action(instance=None):
    ...
    def process_assistant_command(self, user_command, lbl_status):
        if not user_command:
            lbl_status.text = "ხმა ვერ იქნა ამოცნობილი."
            return

        lbl_status.text = f"მუშავდება: \"{user_command}\"..."
        url = f"{VERCEL_BASE_URL}/api/index"
        payload = {
            "text": f"You are LingoLens Assistant. Respond concisely in Georgian to: '{user_command}'",
            "source_lang": "ka",
            "target_lang": self.target_lang[:2],
            "mode": "standard"
        }
        ...
        def _on_assistant_reply(success, response):
            if success and isinstance(response, dict):
                ...
                lbl_status.text = "შეცდომა პასუხის მიღებისას."
    ...
    def translate_with_grammar(self):
        ...
        if not text:
            return
        self.ids.output_text.text = "[AI გრამატიკული ანალიზი...]"
        url = f"{VERCEL_BASE_URL}/api/index"
        payload = {
            "text": f"Grammatically analyze this in Georgian: {text}",
            "source_lang": self.source_lang[:2],
            "target_lang": self.target_lang[:2],
            "mode": "grammar"
        }
        ...
        def _on_result(success, response):
            if success and isinstance(response, dict):
                translated = response.get('translated_text', '')
                grammar = response.get('grammar_analysis', translated)
                self.ids.output_text.text = f"თარგმანი:\n{translated}\n\n--- გრამატიკა ---\n{grammar}"
                db.add_history(self.source_lang, self.target_lang, text, translated)
            else:
                self.ids.output_text.text = "[შეცდომა გრამატიკის ანალიზში]"

        AsyncTranslateEngine.async_post_request(url, payload, _on_result)

    def start_speech_to_text(self, lang_code="ka", on_complete_callback=None):
        self.stt_callback = on_complete_callback
        if platform == 'android':
            try:
                ...
                def _on_activity_result(request_code, result_code, intent):
                    if request_code == 1001 and result_code == -1 and intent:
                        ...
                        if results and results.size() > 0:
                            ...
                            Clock.schedule_once(lambda dt: self._set_speech_input(spoken_text, self.stt_callback), 0)
                ...
                def _launch_stt():
                    ...
                    stt_intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM)
                    stt_intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE, lang_code)
                    activity.startActivityForResult(stt_intent, 1001)

                _launch_stt()
            except Exception as e:
                print(f"STT Exception: {e}")
        else:
            self.ids.input_text.text = "[ხმოვანი შეყვანა ხელმისაწვდომია Android-ზე]"
    ...
    def _download_and_play_tts_stream(self, text, lang_code):
        try:
            ...
            filepath = os.path.abspath(os.path.join(cache_dir, f"speech_{abs(hash(text))}.mp3"))

            res = requests.post(f"{VERCEL_BASE_URL}/api/tts", json={"text": text, "lang": lang_code}, timeout=12)
    ...
    def open_history_popup(self):
        ...
        popup = Popup(title="ნათარგმნი ისტორია", title_font=FONT_PATH, content=content, size_hint=(0.9, 0.8))
