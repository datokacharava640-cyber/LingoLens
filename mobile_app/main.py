...
class MainScreen(Screen):
    ...
    def open_language_menu(self, mode):
        ...
        search_input.bind(text=lambda instance, val: populate_languages(val))
        populate_languages()
    ...
    def on_live_translate(self, text):
        ...
        if not text.strip():
            ...
            return
        self._debounce_event = Clock.schedule_once(lambda dt: self.trigger_translation(text), 0.6)
    ...
    def trigger_translation(self, text):
        ...
        AsyncTranslateEngine.async_post_request(url, payload, self._on_translation_response)

    def _on_translation_response(self, success, result):
        if success and isinstance(result, dict) and "translation" in result:
            translated = result["translation"]
            self.ids.output_text.text = translated
            db.add_history(self.source_lang, self.target_lang, self.ids.input_text.text, translated)
        else:
            cached = db.search_offline_cache(self.source_lang, self.target_lang, self.ids.input_text.text)
            if cached:
                self.ids.output_text.text = cached
            else:
                self.ids.output_text.text = f"[ონლაინ სერვერთან კავშირი ვერ დამყარდა]\n{result}"
    ...
    def analyze_and_think(self):
        ...
        if not text:
            ...
            return
        reasoning_report = ReasoningEngine.deep_reasoning_analysis(text, self.source_lang, self.target_lang)
        self.ids.output_text.text = reasoning_report

    def start_speech_to_text(self, lang):
        trigger_vibration()
        self.ids.audio_viz.start_animation()
        NativeSpeechManager.start_listening(lang, callback_text=lambda t: setattr(self.ids.input_text, 'text', t))
    ...
    def speak_output_text(self):
        ...
        if text:
            NativeSpeechManager.speak_text(text, self.target_lang, speed=self.tts_speed)

    def toggle_tts_speed(self):
        speeds = [1.0, 1.25, 1.5, 0.75]
        curr_idx = speeds.index(self.tts_speed) if self.tts_speed in speeds else 0
    ...
    def share_translation(self):
        ...
        if text:
            try:
                share.share(text=text, title="LingoLens Translation")
            except Exception as e:
                log_error(e)

    def save_to_favorites(self):
        trigger_vibration()
        src_text = self.ids.input_text.text.strip()
        tgt_text = self.ids.output_text.text.strip()
        if src_text and tgt_text:
            if db.add_favorite(self.source_lang, self.target_lang, src_text, tgt_text):
    ...
    # ----------------------------------------------------
    # დამატებითი ფანჯრების გახსნის ლოგიკა
    # ----------------------------------------------------
    def open_interpreter_mode(self):
        trigger_vibration()
        layout = BoxLayout(orientation='vertical', padding=10, spacing=8)
        log_box = TextInput(readonly=True, font_name=FONT_PATH, font_size='14sp', background_color=(0.1, 0.1, 0.15, 1), foreground_color=(1, 1, 1, 1))
        
        btn_p1 = Button(text="🗣️ პირი A (ქართული)", font_name=FONT_PATH, height='45dp', size_hint_y=None, background_color=(0.2, 0.6, 0.4, 1))
        btn_p2 = Button(text="🗣️ პირი B (English)", font_name=FONT_PATH, height='45dp', size_hint_y=None, background_color=(0.2, 0.4, 0.7, 1))
        
        btn_p1.bind(on_release=lambda x: NativeSpeechManager.start_listening("ka", lambda t: log_box.insert_text(f"\n[A]: {t}")))
        btn_p2.bind(on_release=lambda x: NativeSpeechManager.start_listening("en", lambda t: log_box.insert_text(f"\n[B]: {t}")))

        layout.add_widget(log_box)
        layout.add_widget(btn_p1)
        layout.add_widget(btn_p2)

        popup = Popup(title='🗣️ ცოცხალი დიალოგის რეჟიმი', title_font=FONT_PATH, content=layout, size_hint=(0.9, 0.8))
        popup.open()

    def open_dictionary_hub(self):
        trigger_vibration()
        layout = BoxLayout(orientation='vertical', padding=10, spacing=8)
        search = TextInput(hint_text="🔍 მოძებნეთ ლექსიკონში...", font_name=FONT_PATH, height='40dp', size_hint_y=None, multiline=False)
        res_area = TextInput(readonly=True, font_name=FONT_PATH, font_size='14sp', background_color=(0.1, 0.1, 0.15, 1), foreground_color=(0, 0.9, 0.8, 1))

        def lookup(val):
            val_clean = val.strip().lower()
            if val_clean in OFFLINE_DICTIONARY:
                d = OFFLINE_DICTIONARY[val_clean]
                res_area.text = f"📖 სიტყვა: {val_clean}\n• თარგმანი: {d['ka']}\n• მეტყველების ნაწილი: {d['pos']}\n• სინონიმები: {', '.join(d['syn'])}\n• ანტონიმები: {', '.join(d['ant'])}\n• იდიომა: {d['idiom']}"
            else:
                res_area.text = "სიტყვა ვერ მოიძებნა ოფლაინ ბაზაში."

        search.bind(text=lambda inst, val: lookup(val))
        layout.add_widget(search)
        layout.add_widget(res_area)

        popup = Popup(title='📖 ოფლაინ ლექსიკონი & იდიომები', title_font=FONT_PATH, content=layout, size_hint=(0.9, 0.8))
        popup.open()
    ...
    def open_quiz_mode(self):
        ...
        layout = BoxLayout(orientation='vertical', padding=10, spacing=8)
        lbl_q = Label(text=item['q'], font_name=FONT_PATH, font_size='16sp', size_hint_y=None, height='50dp')
        layout.add_widget(lbl_q)

        popup = Popup(title='📝 ყოველდღიური ენობრივი ქვიზი', title_font=FONT_PATH, content=layout, size_hint=(0.85, 0.6))

        def check_ans(opt):
            if opt == item['a']:
                lbl_q.text = "✅ სწორია! ყოჩაღ!"
                lbl_q.color = (0.2, 0.9, 0.3, 1)
            else:
                lbl_q.text = f"❌ არასწორია! სწორია: {item['a']}"
                lbl_q.color = (0.9, 0.2, 0.2, 1)

        for opt in item['options']:
            btn = Button(text=opt, font_name=FONT_PATH, height='40dp', size_hint_y=None, background_color=(0.2, 0.3, 0.4, 1))
            btn.bind(on_release=lambda inst, o=opt: check_ans(o))
    ...
    def open_file_translator(self):
        ...
        try:
            filechooser.open_file(on_selection=_on_file_selected)
        except Exception as e:
            log_error(e)
